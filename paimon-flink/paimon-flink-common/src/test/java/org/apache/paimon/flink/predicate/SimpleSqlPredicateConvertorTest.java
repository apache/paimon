...
class SimpleSqlPredicateConvertorTest {
    ...
    public void testSql() throws SqlParseException {
        String conditionSql =
                "b is null and c is not null and 100<> a and not (a > 100)  or a >1 or a<2 or a=1 or a in ('1','2')";

        Predicate predicate = simpleSqlPredicateConvertor.convertSqlToPredicate(conditionSql);
    ...
    public void testSqlNoSupport(){
        ...
        Assertions.assertThatThrownBy(()-> simpleSqlPredicateConvertor.convertSqlToPredicate("substring(f,0,1) =1"))
                .hasMessage("SUBSTRING(`f` FROM 0 FOR 1) or 1 not been supported.");
        // like not supported
        Assertions.assertThatThrownBy(()-> simpleSqlPredicateConvertor.convertSqlToPredicate("b like 'x'"))
