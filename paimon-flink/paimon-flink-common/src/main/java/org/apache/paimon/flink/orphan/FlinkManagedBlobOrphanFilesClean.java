...
public class FlinkManagedBlobOrphanFilesClean extends ManagedBlobOrphanFilesClean {
    ...
    public DataStream<CleanOrphanFilesResult> doClean(StreamExecutionEnvironment env) {
        ...
        if (frozen.abort) {
            return env.fromCollection(
                    Collections.singletonList(new CleanOrphanFilesResult(0, 0)),
    ...
    private void configureEnv(StreamExecutionEnvironment env) {
        ...
        // Bounded two-input operators fully consume the first input before the second. A pipelined
        // shuffle can deadlock that build/probe order, so force blocking exchanges.
    ...
    private FrozenUsedPacks freezeUsedPacks(StreamExecutionEnvironment env) {
        ...
        // executeAndCollect materializes abort flags and used names together. Deletion re-sources
        // the frozen names and must not reconnect the mark DAG: a lost used-pack shuffle with a
        // surviving sidecar shuffle can drop a reused pack and still agree. Cost: client memory
        // grows with the used-pack set. An OOM fails the job instead of deleting live packs.
        ...
        if (abort) {
            LOG.warn(
                    "Skip managed blob pack GC for table {} because sidecars, manifests, or snapshot topology cannot be trusted, or the used pack set changed during collection.",
                    table.fullName());
        } else {
            LOG.info(
                    "Frozen {} used managed blob pack names for table {} before deletion.",
                    used.size(),
                    table.fullName());
        }
    ...
    private DataStream<CleanOrphanFilesResult> deleteUnused(
        ...
        DataStream<Boolean> skipGc = candidates.getSideOutput(candidateSkipGcTag);
                ...
                unused.keyBy(candidate -> candidate.f0)
                        ...
                        .transform(
                                ...
                                new BoundedTwoInputOperator<
                                    ...
                                    private final Set<String> completedCandidates = new HashSet<>();
                                    ...
                                    public void processElement1(
                                        ...
                                        if (completedCandidates.contains(candidate.f0)) {
                                            return;
                                        }
    ...
    // Re-source frozen used identities so deletion does not reconnect the mark DAG. Keep the source
    // serial so each worker does not deserialize the full used set; the keyed anti-join retains
    ...
    private DataStream<Tuple3<String, String, Long>> unusedCandidates(
        ...
        DataStream<String> frozenNames =
                env.fromCollection(new ArrayList<>(frozenUsed), STRING_TYPE_INFO)
    ...
    public static CleanOrphanFilesResult executeDatabase(
        ...
        // Freeze+delete must finish for one table before the next table's operators are added to
        // the same environment. Unioning table graphs would let a later freeze executeAndCollect
    ...
    // Keyed bounded anti-join: build used identities from the lineage-detached source, then emit
    ...
    private static final class UnusedCandidateJoinOperator
            extends BoundedTwoInputOperator<
                    String, Tuple3<String, String, Long>, Tuple3<String, String, Long>> {
        ...
        public void endInput(int inputId) {
            switch (inputId) {
                case 1:
                    ...
                    LOG.info("Finish build phase for frozen used managed blob packs.");
                    buildEnd = true;
                    break;
                case 2:
                    checkState(buildEnd, "Should build ended.");
                    LOG.info("Finish probe phase for managed blob candidates.");
                    break;
            }
        }
        ...
        @Override
        public void processElement2(StreamRecord<Tuple3<String, String, Long>> element) {
            checkState(buildEnd, "Should build ended.");
            Tuple3<String, String, Long> candidate = element.getValue();
