...
public class PulsarLogSerializationSchema implements PulsarSinkSerializationSchema<SinkRecord> {
    ...
    @Override
    public PulsarMessage<byte[]> serialize(SinkRecord element) {
        ...
        if (primaryKeyBytes != null) {
            builder.key(new String(primaryKeyBytes));
        }
        builder.value(Schema.BYTES, getSchema().encode(valueBytes));
        builder.property(PAIMON_BUCKET, String.valueOf(element.bucket()));
