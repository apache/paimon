...
public class VariantUtils {
    ...
    public static DataField[] replaceWithShreddingFields(
            DataField[] readFields, RowType[] shreddingSchemas) {
        DataField[] replacedFields = new DataField[readFields.length];
        for (int i = 0; i < readFields.length; i++) {
            DataField field = readFields[i];
            if (shreddingSchemas[i] != null) {
                replacedFields[i] = field.newType(shreddingSchemas[i]);
            } else {
                replacedFields[i] = field;
            }
        }
        return replacedFields;
    }

    public static VariantSchema[] toVariantSchemas(RowType[] shreddingSchemas) {
        return Arrays.stream(shreddingSchemas)
                .map(
                        schema ->
                                schema == null
                                        ? null
                                        : PaimonShreddingUtils.buildVariantSchema(schema))
                .toArray(VariantSchema[]::new);
    }
