...
public class PhysicalWriterImpl implements PhysicalWriter {
    ...
    public PhysicalWriterImpl(PositionOutputStream out, OrcFile.WriterOptions opts)
        ...
        this.blockOffset = 0;
        this.unencrypted = new VariantTracker(opts.getSchema(), compress);
        this.writeVariableLengthBlocks = opts.getWriteVariableLengthBlocks();
        this.shims = opts.getHadoopShims();
        this.rawStream = new DirectStream(out);
        this.compressStream = new OutStream("stripe footer", compress, rawStream);
        this.codedCompressStream = CodedOutputStream.newInstance(compressStream);
    }
    ...
    protected static class VariantTracker {
         ...
         */
        public void writeStreams(StreamName.Area area, PositionOutputStream raw)
                throws IOException {
            for (Map.Entry<StreamName, BufferedStream> stream : streams.entrySet()) {
    ...
    private void writeStripeFooter(
        ...
        dirEntry.setFooterLength(out.getPos() - stripeStart - sizes.total());
    }
    ...
    public void writeFileMetadata(OrcProto.Metadata.Builder builder) throws IOException {
        long stripeStatisticsStart = out.getPos();
    ...
    public void writeFileFooter(OrcProto.Footer.Builder builder) throws IOException {
        ...
        long bodyLength = out.getPos() - metadataLength - stripeStatisticsLength;
        builder.setContentLength(bodyLength);
        builder.setHeaderLength(headerLength);
        long startPosn = out.getPos();
        OrcProto.Footer footer = builder.build();
        footer.writeTo(codedCompressStream);
        codedCompressStream.flush();
        compressStream.flush();
        this.footerLength = (int) (out.getPos() - startPosn);
    }
    ...
    public long writePostScript(OrcProto.PostScript.Builder builder) throws IOException {
        ...
        long startPosn = out.getPos();
        ps.writeTo(out);
        long length = out.getPos() - startPosn;
        if (length > 255) {
    ...
    public void finalizeStripe(
        ...
        unencrypted.writeStreams(StreamName.Area.INDEX, out);
        // write the encrypted index streams
        for (VariantTracker variant : variants.values()) {
            variant.writeStreams(StreamName.Area.INDEX, out);
        }

        // write the unencrypted data streams
        unencrypted.writeStreams(StreamName.Area.DATA, out);
        // write out the unencrypted data streams
        for (VariantTracker variant : variants.values()) {
            variant.writeStreams(StreamName.Area.DATA, out);
        }
    ...
    public void writeHeader() throws IOException {
        out.write(OrcFile.MAGIC.getBytes());
        headerLength = out.getPos();
    }
