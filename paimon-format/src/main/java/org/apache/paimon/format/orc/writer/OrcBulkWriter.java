...
public class OrcBulkWriter implements FormatWriter {
    ...
    public void addElement(InternalRow element) throws IOException {
        synchronized (this) {
            long elementSize = 0;
            if (vectorizer instanceof RowDataVectorizer) {
                elementSize =
                        ((RowDataVectorizer) vectorizer).vectorizeAndGetBytes(element, rowBatch);
                currentBatchMemoryUsage += elementSize;
            } else {
                vectorizer.vectorize(element, rowBatch);
            }

            if (rowBatch.size == rowBatch.getMaxSize()
                    || currentBatchMemoryUsage >= this.memoryLimit) {
                flush();
            }
        }
    }
