...
public class LanceUtils {
    ...
    public static Pair<Path, Map<String, String>> toLanceSpecified(FileIO fileIO, Path path) {
        ...
        if (fileIO instanceof OSSFileIO) {
            originOptions = ((OSSFileIO) fileIO).hadoopOptions();
        } else if (fileIO instanceof JindoFileIO) {
            originOptions = ((JindoFileIO) fileIO).hadoopOptions();
        } else {
            originOptions = new Options();
        }

        if (fileIO instanceof OSSFileIO || fileIO instanceof JindoFileIO) {
            URI uri = path.toUri();
            storageOptions.put(
                    "endpoint", "https://" + uri.getHost() + originOptions.get("fs.oss.endpoint"));
