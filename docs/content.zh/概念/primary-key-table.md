...
A bucket is the smallest storage unit for reads and writes, each bucket directory contains an [LSM tree]({{< ref "概念/文件布局#lsm-trees" >}}).
...
[Sequence Field]({{< ref "概念/primary-key-table#sequence-field" >}}) to correct disorder.
{{< /hint >}}
...
{{< hint info >}}
For streaming queries, `partial-update` merge engine must be used together with `lookup` or `full-compaction` [changelog producer]({{< ref "概念/primary-key-table#changelog-producers" >}}).
{{< /hint >}}
...
{{< hint info >}}
For streaming queries, `aggregation` merge engine must be used together with `lookup` or `full-compaction` [changelog producer]({{< ref "概念/primary-key-table#changelog-producers" >}}).
{{< /hint >}}

## Changelog Producers

Streaming queries will continuously produce the latest changes. These changes can come from the underlying table files or from an [external log system]({{< ref "概念/external-log-systems" >}}) like Kafka. Compared to the external log system, changes from table files have lower cost but higher latency (depending on how often snapshots are created).
...
By specifying `'changelog-producer' = 'input'`, Paimon writers rely on their inputs as a source of complete changelog. All input records will be saved in separated [changelog files]({{< ref "概念/文件布局" >}}) and will be given to the consumers by Paimon sources.
