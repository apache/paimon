...
Access a specific branch of the table:
...
schema = Schema.from_pyarrow_schema(
    ...
    options={
        'bucket': '2',
        CoreOptions.BRANCH.key(): 'feature_branch'
    }
)

catalog.create_table('default.branched_table', schema, False)

# Access table on specific branch
table = catalog.get_table('default.branched_table')
current_branch = table.current_branch()  # Returns 'feature_branch'
```
