...
Batch scans use regular split packing by default. To opt into bucket distribution for storage
partition joins, see [Scan Layout and Storage Partition Joins](./sql-query#scan-layout-and-storage-partition-joins).
