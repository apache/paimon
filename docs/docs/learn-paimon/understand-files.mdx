...
2. [Primary Key Table](../primary-key-table/overview) and [Append Table](../append-table/overview)
...
By default, sorted runs number depends on `num-sorted-run.compaction-trigger`, see [Compaction for Primary Key Table](../primary-key-table/compaction),
