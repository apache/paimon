...
V2 writes the payloads first, followed by the footer containing the index entries and a fixed
12-byte trailer.
...
values. Payloads are written through `FileIndexWriter.writeTo(OutputStream)`; existing byte-array
writers use its default implementation. `FileIndexer.createReader` receives both the start position
and length as 64-bit values. Index implementations that load a payload into a `byte[]`, such as
Bloom filter, may reject lengths greater than `Integer.MAX_VALUE`.
