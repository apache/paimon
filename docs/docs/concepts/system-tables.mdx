...
| --- | --- | --- |
| `partition` | STRING | Partition of the data file. |
| `bucket` | INT | Bucket of the data file. |
| `file_path` | STRING | Path of the data file. |
| `file_size_in_bytes` | BIGINT | Size of the data file. |
| `record_count` | BIGINT | Number of records in the data file. |
| `schema_id` | BIGINT | Schema ID of the data file. |
| `column_name` | STRING | Indexed column. Nested map indexes use names such as `attributes[country]`. |
| `index_type` | STRING | File index type, such as `bitmap`, `bloom-filter`, or `range-bitmap`. |
| `storage_type` | STRING | `EMBEDDED` when stored in the manifest, otherwise `FILE`. |
| `index_file_path` | STRING | Path of the external index container, or `NULL` for an embedded index. |
| `index_size_in_bytes` | BIGINT | Size of this column and index-type payload. |
| `index_container_size_in_bytes` | BIGINT | Total size of the shared index container. |
| `is_empty` | BOOLEAN | Whether this index has an empty payload. |
