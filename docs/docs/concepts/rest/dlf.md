...
`x-acs-action` header naming the DlfNext API that the request's method and path map to, which some
gateways require before they route the call. That signer is opt-in: no endpoint selects it on its
own. The client also infers the region from the URI.
