...
- Deletion vector is only supported for chain table with the `DEDUPLICATE` merge engine.
...
- Chain table requires `sequence.field`, so the `FIRST_ROW` and `AGGREGATE` merge engines are not supported.
- For chain table delta computation, `partition.timestamp-formatter` must specify a complete date
  (year-month-day or year-day-of-year), only use round-trippable date/time fields
  (`y/u`, `M/L`, `d`, `D`, `H`, `k`, `m`, `s`), and the minimum step must be at least one second.
