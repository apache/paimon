...
- The `changelog-producer` option must be `none` (default) or `input`; `lookup` and `full-compaction` are not supported for chain tables.
...
1. **Full load phase**: By default it produces a lightweight result by reading the latest
   ...
   anchor-based chain merging in Phase 1, allowing cross-branch deletes and updates to be
   visible in the starting snapshot.
...
- **Delta branch** receives incremental changes between snapshots (e.g., a batch job writes the
  ...
  contains only the changes for that period.
...
### Merge Snapshot in Starting Phase

By default, the full load phase is lightweight: for each group it reads the latest
snapshot partition and later delta partitions as separate splits. Anchor-based chain
merging is skipped to keep Phase 1 fast and avoid split explosion in jobs with many
partitions.

If the delta branch contains deletes or updates that must be merged with the latest
snapshot anchor — for example, a later delta partition deletes a key that exists in the
latest snapshot partition — you can enable the merge mode:
...
When enabled, the starting phase merges the latest snapshot partition per group with
delta partitions whose chain key is strictly greater than the snapshot chain key. This
produces a correct starting snapshot that reflects cross-branch deletes and updates,
at the cost of a heavier Phase 1.

To reduce this overhead, you can periodically run `compact_chain_table` to merge delta
partitions into the snapshot branch. After compaction, the snapshot branch contains the
merged state, so the streaming starting phase can read snapshot partitions directly
without expensive cross-branch merging.
...
- The chain-table-aware streaming scan only supports the default startup mode (`latest-full`).
  ...
  `scan.timestamp-millis`, `scan.mode = 'latest'`, or `consumer-id` — an
  `UnsupportedOperationException` is thrown. To use standard streaming read without chain table
  logic, read from a specific branch table (e.g., `t$branch_delta`) instead of the main table.
