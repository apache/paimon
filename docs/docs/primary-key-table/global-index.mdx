...
| `fields.<column>.pk-vector.index.type` | Required | ANN implementation, such as `ivf-flat`, `ivf-pq`, `ivf-sq`, `ivf-rq`, or `diskann`. |
...
in every non-zero data level. Most families write one payload per group; FM can write several
ordered partition payloads. When data compaction changes a level, Paimon rebuilds that whole level
group, including files in the target level which were not direct compaction inputs. A level group
is used only when its ordered source names and row counts exactly match the current data level and
its payload row ranges cover the level exactly; gaps, overlaps, stale payloads, and cross-level
