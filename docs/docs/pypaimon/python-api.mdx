...
Use path helpers when only a few scalar fields change. They avoid decoding
unrelated fields into Python objects:

```python
import operator

from pypaimon.data import variant_transform

updated_payload = variant_transform(payload, {
    '$.velocity.y': operator.neg,
    '$.velocity.z': operator.neg,
})
```

`variant_get` extracts one path as an Arrow array. `variant_get_many` and
`variant_set_many` process several paths in one pass. `variant_set` replaces
one path. Set operations require existing paths and scalar replacement values;
they keep the original VARIANT metadata.
