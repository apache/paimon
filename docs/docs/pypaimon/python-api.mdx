...
Currently, PyPaimon supports filesystem catalog, jdbc catalog and rest catalog. See [Catalog](../concepts/catalog).
...
| WKB `bytes`         | GeoArrow WKB extension or `pyarrow.large_binary()` | `GEOMETRY(crs)`, `GEOGRAPHY(crs, algorithm)` |

For geospatial fields, PyPaimon follows PyIceberg and uses the standard GeoArrow WKB extension type when the optional
`geoarrow-pyarrow` package is installed (`pip install pypaimon[geoarrow]`). The extension metadata preserves the CRS
and edge-interpolation algorithm, including for fields nested in arrays, maps, and rows. Without GeoArrow, PyPaimon
falls back to `pyarrow.large_binary()` and relies on the Paimon table schema for the logical type; an unannotated Arrow
binary field is inferred as `BYTES` or `BLOB`. PyArrow does not currently expose the native Parquet `GEOMETRY` and
`GEOGRAPHY` logical annotations required by Paimon and Iceberg v3, so PyPaimon can represent and read these fields but
rejects table writes containing them.
Geospatial tables require Parquet, and non-spherical geography algorithms may not be supported by every query engine.
