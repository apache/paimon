...
Read a path as an Arrow array, use Arrow compute, and replace its existing
values without decoding unrelated fields:
...
`variant_get` returns NULL for a missing path. `variant_replace` accepts an
Arrow Scalar, Array, or ChunkedArray. A missing path is unchanged unless
`strict=True` is specified. Pass `{path: type}` to `variant_get` and
`{path: values}` to `variant_replace` to process multiple paths in one pass.
