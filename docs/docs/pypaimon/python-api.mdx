...
### VARIANT Numeric Updates

Transform existing FLOAT or DOUBLE paths without decoding unrelated fields:
...
The paths must exist and contain FLOAT or DOUBLE values. Each transform must
return a `float`; the original numeric type and metadata are unchanged.
