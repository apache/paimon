...
`join_asof` follows the binary as-of join model used by pandas, Polars, and
DuckDB. It preserves each left row and matches at most one right row within the
...
batches from the snapshots resolved when `join_asof` is created. BLOB values
