...
Video imports keep each Episode in one aligned file group. When a target is
reached, normal, camera sidecar, and vector files roll together at the next
Episode boundary. The target table must be unpartitioned and bucket-unaware.
