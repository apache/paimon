...
Because native HDFS Kerberos credentials use process-global ticket state, an
HDFS source configured with an explicit keytab must use the same principal as
an HDFS target. A different or unverifiable target principal is rejected before
source discovery; isolate such a load in another process instead.
