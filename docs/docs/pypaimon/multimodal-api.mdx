...
The official LeRobot reader requires a local root. URI sources are therefore
copied once to a temporary directory, imported, and removed. Each MP4 is copied
once, not once per frame; ensure local temporary storage can hold the source.
