...
and missing keys append new rows. BlobStore does not enforce object-key
uniqueness; that should be modeled with a table-level unique key when available.
The object body can be raw bytes, a binary file-like object, or a PyPaimon
`Blob`. Use `columns` to set the non-key, non-BLOB table columns for the object
row. For managed BLOB storage, use `head_object` or `get_object` to inspect the
stored descriptor after the write.
