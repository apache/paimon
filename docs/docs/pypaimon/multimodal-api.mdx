...
list. `delete_object` deletes one key; `delete_objects` deletes a batch and
folds repeated keys in the request.
...
store.delete_objects(["images/old-1.jpg", "images/old-2.jpg"])
```
