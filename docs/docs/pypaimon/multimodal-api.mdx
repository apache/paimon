...
`append_hdf5` streams one or more local or remote HDF5 files into an existing
multimodal table. Install the optional dependency first:
...
Directories are searched recursively. Before opening files, `append_hdf5`
...
```python
remote_result = frames.append_hdf5(
...
HDF5 requires random access. `append_hdf5` passes the seekable stream returned
...
commit starts has Paimon's unknown-commit-result semantics: `append_hdf5` does
...
writer commit messages for one coordinator commit. Do not call `append_hdf5`
independently in every task, because that would create one commit per task. The
HDF5 core itself has no Ray dependency.

`append_hdf5` is append-only and is **not retry-safe**. It does not add source
