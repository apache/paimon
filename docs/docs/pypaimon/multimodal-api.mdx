...
`task_index`. A manifest row is reserved as `PENDING` and marked `READY` after
...
reading the same tag from its component tables reconstructs that release.
The one-time importer requires a new target table. A failed import removes the
table group so the call can be retried. `drop_table()` removes the table group;
