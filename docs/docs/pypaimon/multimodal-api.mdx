...
Use `align` to assemble a training timeline from independently sampled tables.
The anchor scan defines one output row per step. Named secondary scans match
within the same episode (or other `by` keys) by exact, backward, forward, or
nearest timestamp.
...
scans. All floating key values and tolerances must be finite. Numeric timestamps
use numeric tolerances (integer tolerances must fit `int64`); Arrow timestamps
use `datetime.timedelta`. Tolerance is inclusive, and nearest ties choose the
earlier row. Queries that mask `_ROW_ID` are not supported.

Planning builds a compact Arrow index from grouping keys, timestamps, and row
IDs. Selected payload rows, including nested projections, are then fetched in
batches from the same snapshots resolved when `align` is created. BLOB values
remain descriptors, allowing the existing BLOB and video readers to fetch or
decode only selected samples. Payload rows are fetched per output batch, while
the coordinator-local metadata index grows with the selected rows. Alignment
does not interpolate values or build training windows; those operations remain
explicit downstream steps.
