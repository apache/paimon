...
If `version_id` is omitted, the latest published version is used. Its info,
episodes, tasks, subtasks, and stats are available through `dataset.meta`.
Payload columns remain lazy. Pass
