...
For map-style training, read a published version directly from Paimon:
...
If `version_id` is omitted, the latest published version is used. Metadata is
available through `dataset.meta`. Payload columns remain lazy. Pass
`index_mapping=dataset.index_mapping` to readers of the same table snapshot to
avoid rescanning the control columns.
