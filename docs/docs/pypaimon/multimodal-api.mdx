...
Paimon commit. Both methods overwrite existing rows with the same keys. The
object body can be raw bytes, a binary file-like object, or a PyPaimon `Blob`.
Use `columns` to set the non-key, non-BLOB table columns for the object row.
