...
`VECTOR`, higher-rank tensors to nested `ARRAY`, and media to `BLOB`. Images
keep their compressed bytes; video frames are stored as PNG BLOBs while only
active MP4 chunks are cached locally during import.

`feature_mapping` renames columns; `transform` maps Arrow batches without
changing frame order. Only v3 is supported; `uint64`, depth video, and language
event structures are rejected.
