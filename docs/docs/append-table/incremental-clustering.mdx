...
For the partitioned `my_table` example, add `--partition dt=2026-09-10` to limit the job to one partition.
...
For unaware-bucket tables, a clustering run can also select inactive historical partitions for full clustering. This
happens as part of the submitted job; configuring the options does not start a scheduler.
...
| `clustering.history-partition.limit` | `5` | Maximum number of historical partitions selected for automatic full clustering in a run. |
...
The idle duration enables the feature; the limit is optional. The planner evaluates eligible partitions and selects
those that need full clustering. These options do not apply to bucketed append tables.
