...
|---|---|---|
| `sorted-index.records-per-file` | `25000000` | Expected number of source records per generated Multivalue index file. |
...
element's posting list in memory. `sorted-index.records-per-file` bounds the source-row range—and
