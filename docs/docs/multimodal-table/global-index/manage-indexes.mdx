...
| `data-evolution.scalar-index.max-selection-ratio` | `0.2` | Maximum candidate row-ID ratio for an index-assisted Data Evolution scalar batch scan, using the snapshot's allocated row-ID count as the denominator. Broader results fall back to a regular data scan. |
...
| `data-evolution.scalar-index.max-decoded-row-ids` | `10000000` | Maximum row IDs a supporting scalar reader may decode for one indexed field. Currently enforced by BTree; compressed bitmaps that remain compressed are not counted. |
