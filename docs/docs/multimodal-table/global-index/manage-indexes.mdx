...
| `global-index.query-in-reader.enabled` | `false` | Query supported scalar global indexes per data split in readers instead of materializing index results during scan planning. |
...
| `sorted-index.records-per-file` | `25000000` | Expected number of records per sorted global index file for BTree, Bitmap, and Multivalue builds. Falls back to `sorted-index.records-per-range`, then `btree-index.records-per-range`. |
...
The reader-side query option applies to supported scalar indexes on data evolution tables. It is
evaluated by the core scan, including scans used by Spark. Flink's
`scan.dedicated-split-generation` mode evaluates the index in the dedicated split-generation task
instead of data readers, even when `global-index.query-in-reader.enabled` is true.
