...
is a reader-side work budget and currently applies to BTree posting lists; compressed
...
2. Readers enforce work-specific limits. Physical bytes read from BTree, Bitmap,
   Multivalue, and FM index files share the same byte budget. BTree row-ID lists and
...
| `data-evolution.scalar-index.max-read-bytes` | `256 mb` | Maximum physical index bytes read for one indexed field. Cached bytes are not charged. |
| `data-evolution.scalar-index.max-total-read-bytes` | `512 mb` | Aggregate physical index bytes read across indexed fields in one scalar batch query. |
