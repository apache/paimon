...
Non-partition predicates are resolved into an exact set of matching rows
**before** ranking, and that set is handed to the existing full-text index, so
BM25 statistics are always those of the full corpus:
