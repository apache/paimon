...
The writer divides source rows into independent partitions. Each partition stores a compressed,
checksummed wavelet matrix, sampled suffix-array values, row boundaries, null rows, and exact
verification pages. Reads demand-load bounded blocks instead of downloading the complete index.
If locating matches would cost more than exact verification, the reader scans the relevant
verification pages and still returns an exact result.
...
partitions searched per query. A lower `fm-index.sa-sample-rate` accelerates locating matched rows
but stores more suffix-array samples.
