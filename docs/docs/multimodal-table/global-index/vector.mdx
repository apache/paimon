...
The procedure returns JSON-serialized rows as strings, ordered by score from highest to lowest.
...
`where` is evaluated before Top-K. An omitted or blank `partitions` value means no partition
restriction. A non-blank value accepts semicolon-separated complete partition specs; empty specs
and duplicate keys within one spec are rejected. If both arguments contain partition conditions,
their intersection is used for planning and final row lookup.

The procedure resolves `scan.version`, snapshot, tag, watermark, or timestamp selection once,
normalizes `scan.mode` to `from-snapshot`, and replaces the selector with the resulting immutable
snapshot ID. Vector planning and row lookup therefore use the same snapshot; a table with no
snapshot returns an empty result. Set
`vector-search.distribute.enabled=true` as a table option or in the procedure's `options` argument
to dispatch sufficiently large index-split or raw-row-range groups to a bounded Flink job. Small
searches retain the local path. The distributed parallelism is capped by the Flink environment
parallelism and `global-index.thread-num`.

For a primary-key vector index, a `where` predicate on non-partition columns requires deletion
vectors with merge-on-read disabled. A `first-row` table has deletion vectors disabled and can use
only partition predicates. See [Primary-Key Global Index](../../primary-key-table/global-index).

The Flink procedure currently rejects tables with `query-auth.enabled=true`. Applying authorization
only while reading the final candidates could return fewer than `top_k` authorized rows, so the
procedure fails before vector planning until authorization predicates can participate in candidate
selection.
