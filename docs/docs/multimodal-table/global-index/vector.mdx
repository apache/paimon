...
`diskann` and `lumina` use different native implementations and option namespaces. `diskann` is
provided by paimon-vindex 0.3.0; `lumina` uses the Lumina integration described separately below.
...
`ivf-sq`, `ivf-rq`, and `diskann`. Use SQL to build Lumina indexes. Install
`paimon-vindex==0.3.0` or `pypaimon[vindex]` before building or querying paimon-vindex indexes from
Python.
...
paimon-vindex indexes or `lumina.index.dimension` for Lumina indexes. For `VECTOR<FLOAT>` columns,
Paimon uses the dimension from the column type. When `lumina.index.dimension` is explicitly set for
a `VECTOR<FLOAT>` column, it must match the vector type length.
...
value for latency-sensitive queries. Do not set both in one query. Lumina query-time options use
the native keys shown above; when the same options are configured as table or index options, use
the `lumina.` prefix.
