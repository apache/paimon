...
For Spark SQL, the second argument is an array of route configs created by `named_struct`.
A route config supports `vector_column`,
...
`__paimon_vector_search_score` to expose the ranked score.
