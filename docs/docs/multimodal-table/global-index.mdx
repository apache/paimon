...
For Java, use `Table.newMultiVectorSearchBuilder()` to configure routes, final limit, and ranker
directly. `VectorSearch` and `MultiVectorSearch` are internal pushdown representations.
