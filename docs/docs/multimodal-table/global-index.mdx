...
Multi-vector search queries multiple vector columns in one request and fuses the scored results
...
| Option | Default | Description |
|---|---|---|
| `fusion` | `rrf` | Fusion strategy for route results. Supported values are `rrf` and `weighted_score`. |
| `route_limit` | Final limit | Top K to retrieve from each vector column before fusion. |
...
final number of fused results to return. Spark SQL adds `__paimon_vector_search_score` to expose
the fused score.
