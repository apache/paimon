...
For `ARRAY<FLOAT>` vector columns, specify the vector dimension with `vector.index.dimension`.
...
|---|---|---|
| `vector.index.dimension` | `128` | Vector dimension for `ARRAY<FLOAT>` columns. Ignored for `VECTOR<FLOAT>` columns. |
| `vector.distance.metric` | `inner_product` | Distance metric. Supported values: `l2`, `cosine`, `inner_product`. |
...
| `vector.hnsw.ef-construction` | `150` | HNSW construction search width for `ivf-hnsw-flat` and `ivf-hnsw-sq`. |
| `vector.hnsw.max-level` | `7` | Maximum HNSW level for `ivf-hnsw-flat` and `ivf-hnsw-sq`. |
...
| `vector.train.sample-ratio` | `1.0` | Ratio of vectors sampled for index training. Must be in `(0, 1.0]`. |
