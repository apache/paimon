...
FROM multi_vector_search(
  'my_table',
  map(
    'title_embedding', array(1.0f, 0.0f, 0.0f),
    'body_embedding', array(0.0f, 1.0f, 0.0f)),
  10,
  map(
    'ranker', 'rrf',
    'candidate_limit', '50',
    'weights', 'title_embedding=2.0,body_embedding=1.0',
    'ivf.nprobe', '16'));
```

The second argument is a map from vector column name to query vector. The third argument is the
final number of ranked results to return. Spark SQL adds `__paimon_vector_search_score` to expose
the ranked score.
