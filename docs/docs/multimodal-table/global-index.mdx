...
- **[ES Index (Multi-Column)](./global-index/es-index)**: A Lucene-backed multi-column index that indexes a vector column together with companion full-text, keyword, scalar, and date columns in a single index.
...
| ES Index | Indexing a vector column together with companion full-text, keyword, scalar, and date columns in one index | Lucene-backed multi-column index. Requires the `paimon-eslib` plugin on the classpath. |
