...
To improve freshness for query types that support slow search, set:

```sql
ALTER TABLE my_table SET ('global-index.fast-search' = 'false');
```

With fast search disabled, supported global-index queries merge indexed results with a scan over
files not covered by global indexes.
...
| `global-index.fast-search` | `true` | Whether global index queries only search indexed files. Set to `false` to also scan files not covered by global indexes when supported. |
