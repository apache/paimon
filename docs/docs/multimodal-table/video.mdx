...
reader fetches these ranges and a bounded GOP window around the target. Zero length selects the
scan fallback. PyPaimon generates indexes for supported ISO BMFF videos and preserves them during
rewrites. See the
