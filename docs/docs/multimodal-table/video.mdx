...
concatenates raw video payloads without ordinary BLOB entry wrappers. Compact indexes map rows to
videos and frame ordinals. Version 2 additionally stores compressed PTS, duration, and key-frame
mappings once per physical video. Consecutive frames share one run entry; NULL and data-evolution
placeholders use negative run references.
...
and rebuilds the embedded indexes; it does not decode or re-encode frames. Persisted frame mappings
...
- Version 1 remains readable but has no persisted frame mapping.
...
PyPaimon records an exact frame mapping while writing supported videos. TorchCodec and PyAV reuse
it to avoid scanning the complete packet stream when a decoder first opens. Missing mappings fall
back to the version 1 behavior.
