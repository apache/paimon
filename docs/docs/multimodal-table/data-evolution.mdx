...
`data-evolution.compaction.rewrite-row-ids` option is no longer supported, and
there is currently no standalone action for physically applying these deletion
vectors.
