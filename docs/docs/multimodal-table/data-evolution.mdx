...
`row-tracking.enabled` and `data-evolution.enabled` in a new schema. No data
file is rewritten. The table must be an append table without primary key,
with `bucket = -1` and without `clustering.incremental`; REST catalog tables
