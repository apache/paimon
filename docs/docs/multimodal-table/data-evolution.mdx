...
`writeCols` value as all table columns. Persist the option with `ALTER TABLE` (or
set it when creating the table); enabling it only as a dynamic job option is
rejected because data files must reference a schema version which records the
compact representation. Before rolling readers back to an older version,
rewrite files produced with this option or keep those readers upgraded.
