...
REST catalog tables are not supported yet. The two options cannot be switched
on by a plain schema change while the table has data files without a row ID.
