...
Use an HTTPS OSS endpoint. The Java implementation used by SQL converts Internal endpoints
to their public equivalents in the returned URL. PyPaimon preserves the configured endpoint,
including Internal endpoints; both the Python process and the URL consumer must be able to
reach that endpoint. For consumers that require public access, configure a public endpoint:
...
PyPaimon exposes the same operation for descriptor-backed `Blob` values. Pass the owning table's
FileIO and storage path so PyPaimon can validate that the descriptor belongs to the table:
...
Install the OSS dependencies with `pip install 'pypaimon[oss]'`. For DLF REST catalogs, PyPaimon
