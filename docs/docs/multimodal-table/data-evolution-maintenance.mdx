...
versioned `snapshot-<snapshotId>-<uuid>.plan` file in the table's `manifest/`
...
snapshot ID; inherited properties on later snapshots are ignored. The file also stores the reassignment snapshot ID
