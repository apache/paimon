...
versioned plan file in the table's `manifest/` directory, together with a
`reassign-snapshot-id` property identifying the snapshot that committed it. The plan
