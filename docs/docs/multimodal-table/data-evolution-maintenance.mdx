...
versioned `snapshot-<snapshotId>-<uuid>.reassign-plan` file in the table's `manifest/`
...
stores the reassignment snapshot ID and the row-ID mappings applied to each affected
