...
versioned plan file in the table's `manifest/` directory. The plan contains the source
and target snapshot IDs and the row-ID mappings for each affected partition. It is
retained while its snapshot or a tag referencing that snapshot is retained.
