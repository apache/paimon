...
versioned plan file in the table's `manifest/` directory. The file stores the
reassignment snapshot ID and the row-ID mappings applied to each affected partition.
It is retained while its snapshot or a tag referencing that snapshot is retained.
