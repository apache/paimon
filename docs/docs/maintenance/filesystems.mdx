...
| Aliyun OSS                   | oss://     | Y         |                                                                        |
| S3                           | s3://, s3a:// | Y      | Both schemes are served by `S3AFileSystem`                             |
...
You can find [How to config jindo sdk on trino](https://github.com/aliyun/alibabacloud-jindodata/blob/master/docs/user/4.x/4.6.x/4.6.12/oss/presto/jindosdk_on_presto) here.
Please note that:
...
Instead of `fs.oss.accessKeyId` and `fs.oss.accessKeySecret`, you can set `fs.oss.credentials.provider` to a class implementing `com.aliyun.oss.common.auth.CredentialsProvider`, for example `com.aliyun.oss.common.auth.EnvironmentVariableCredentialsProvider`. The provider is asked for credentials on every request.

With a REST catalog that vends data tokens, the OSS FileIO signs every request with the catalog's current token, so streams that are already open keep working after the token is refreshed.
