...
public class MergeTreeReaders {
    ...
    public static <T> RecordReader<T> readerForSection(
        ...
        for (SortedRun run : section) {
            readers.add(() -> readerForRun(run, readerFactory, encryptionManager));
        }
    ...
    public static RecordReader<KeyValue> readerForRun(
        ...
        for (DataFileMeta file : run.files()) {
            ...
            if (file.keyMetadata() != null) {
                plaintextKey = encryptionManager.decrypt(file.keyMetadata());
                addPrefix = file.keyMetadata().aadPrefix();
            } else {
                addPrefix = null;
                plaintextKey = null;
            }
