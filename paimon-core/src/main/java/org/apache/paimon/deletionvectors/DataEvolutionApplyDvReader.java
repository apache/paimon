...
/**
 * Applies deletion vectors to data-evolution reads. The underlying reader is asked to include
 * {@code _ROW_ID} when needed, and this reader projects it away again unless the caller requested
 * it.
 */
public class DataEvolutionApplyDvReader implements RecordReader<InternalRow> {
    ...
    @Nullable private RowRangeDeletionVector currentDv;
    ...
    private boolean isDeleted(InternalRow row) {
        ...
        if (currentDv == null || !currentDv.mayContain(rowId)) {
            return false;
        }
    ...
    private void moveToPossibleDv(long rowId) {
        while (currentDv != null && rowId > currentDv.range.to) {
    ...
    public static Info readInfo(
            FileIO fileIO, RowType readRowType, @Nullable Map<Range, DeletionFile> deletionFiles)
            throws IOException {
    ...
    /** RowRange with related deletion vector. */
    private static class RowRangeDeletionVector {
        ...
        private boolean mayContain(long rowId) {
            return rowId >= range.from && rowId <= range.to;
        }
