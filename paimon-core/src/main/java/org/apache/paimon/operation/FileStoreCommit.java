...
public interface FileStoreCommit extends AutoCloseable {
     ...
     * bound of {@link CoreOptions#COMMIT_LAST_SAFE_SNAPSHOT}. The bound only saves the lookup work
     * for a commit user that is created for one run and so cannot have committed before its base
     * snapshot; a caller-provided user that survives a restart can have, and needs the unbounded
     * lookup to recognise a replay. Strict mode checks keep the bound either way.
     */
    FileStoreCommit filterCommittedIgnoresLastSafeSnapshot(boolean ignoresLastSafeSnapshot);
