...
public class LocalManagedBlobOrphanFilesClean extends ManagedBlobOrphanFilesClean
     ...
     *
     * <p>Used-pack collection waits without a timeout. A FileIO call that ignores interruption can
     * block this method indefinitely. {@link #executeDatabase} applies a bounded termination wait
     * only after a sibling table has already failed and remaining tasks are cancelled.
     */
    ...
    private Set<String> getUsedPacks(String branch, ReachabilityScan scan) {
        ...
        try {
            executeSnapshotsInCompletionOrder(
                    ...
                    snapshot -> {
                        try {
                            ...
                            LOG.warn(
                                    "Failed to collect used managed blob packs for table {} branch {} snapshot {}.",
                                    table.fullName(),
                                    branch,
                                    snapshot.id(),
                                    e);
            ...
            LOG.warn(
                    "Failed to list snapshots while collecting used managed blob packs for table {} branch {}.",
                    table.fullName(),
                    branch,
                    e);
            throw new RuntimeException(e);
     ...
     * call can keep running after this method throws. {@code take()} itself has no timeout, so a
     * hung FileIO call also blocks {@link #clean()} until that call returns. This does not leak:
     * every caller reaches {@link #close()} (directly, or via {@link #executeDatabase}, which
     * additionally awaits termination up to {@link #TERMINATION_TIMEOUT_MS}), and that shuts the
     * pool down. Any new caller must preserve that guarantee.
     */
