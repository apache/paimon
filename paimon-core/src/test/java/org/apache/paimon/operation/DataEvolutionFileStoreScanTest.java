...
public class DataEvolutionFileStoreScanTest {
    ...
    @Test
    public void testDedicatedProjectionKeepsEveryNormalDeletionVectorAnchor() {
        DataEvolutionFileStoreScan scan = pruningScan(true, "f1");
        ManifestEntry first = pruningEntry("first.parquet", "f0", 0, 5, 0);
        ManifestEntry second = pruningEntry("second.parquet", "f0", 5, 5, 0);
        ManifestEntry update = pruningEntry("update.parquet", "f0", 0, 5, 1);
        ManifestEntry blob = pruningEntry("spanning.blob", "f1", 0, 10, 0);

        assertThat(scan.postFilterManifestEntries(Arrays.asList(first, second, update, blob)))
                .containsExactlyInAnyOrder(first, second, blob);
    }

    @Test
    public void testMissingColumnProjectionKeepsEverySplitNormalRange() {
        DataEvolutionFileStoreScan scan = pruningScan(false, "f2");
        ManifestEntry first = pruningEntry("first.parquet", "f0", 0, 5, 0);
        ManifestEntry second = pruningEntry("second.parquet", "f0", 5, 5, 0);
        ManifestEntry blob = pruningEntry("spanning.blob", "f1", 0, 10, 0);

        assertThat(scan.postFilterManifestEntries(Arrays.asList(first, second, blob)))
                .containsExactlyInAnyOrder(first, second);
    }

    @Test
    public void testPartialColumnProjectionKeepsMissingSplitNormalRange() {
        DataEvolutionFileStoreScan scan = pruningScan(false, "f2");
        ManifestEntry first = pruningEntry("first.parquet", "f0", 0, 5, 0);
        ManifestEntry second = pruningEntry("second.parquet", "f0", 5, 5, 0);
        ManifestEntry update = pruningEntry("update.parquet", "f2", 0, 5, 1);
        ManifestEntry vector = pruningEntry("spanning.vector.json", "f1", 0, 10, 0);

        assertThat(scan.postFilterManifestEntries(Arrays.asList(first, second, update, vector)))
                .containsExactlyInAnyOrder(update, second);
    }

    private DataEvolutionFileStoreScan pruningScan(boolean deletionVectorsEnabled, String field) {
        TableSchema schema = TableSchema.create(0L, createSchema("f0", "f1", "f2"));
        DataEvolutionFileStoreScan scan =
                new DataEvolutionFileStoreScan(
                        null, null, null, null, schema, null, null, deletionVectorsEnabled);
        scan.withReadType(schema.logicalRowType().project(Collections.singletonList(field)));
        return scan;
    }

    private ManifestEntry pruningEntry(
