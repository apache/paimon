...
class PrimaryKeyBlobFileWriterTest {
    ...
    @Test
    void testPartialUpdateManagedBlobSidecarCollectsIntermediateReferences() throws Exception {
        FileIO fileIO = LocalFileIO.create();
        Path tablePath = new Path(tempDir.toUri());
        RowType keyType =
                new RowType(
        ...
        writer.write(new KeyValue().replace(GenericRow.of(1), 0, RowKind.INSERT, first));
        ...
        writer.write(new KeyValue().replace(GenericRow.of(1), 1, RowKind.INSERT, second));
        ...
        writer.write(
                new KeyValue()
                        .replace(
                                ...
                                2,
                                RowKind.INSERT,
                                GenericRow.of(1, BinaryString.fromString("c"), null)));
        ...
        assertThat(ManagedBlobReferenceFile.read(fileIO, sidecar))
                .containsExactlyInAnyOrder(
                        new ManagedBlobReferenceFile.Reference(
                                sidecar.getParent().toString(), firstPack.getName()),
                        new ManagedBlobReferenceFile.Reference(
                                sidecar.getParent().toString(), secondPack.getName()));
    }
}
