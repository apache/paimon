...
class VideoFileMeta:
    ...
    def __init__(self, stream, file_size: int):
        if file_size < self.V1_FOOTER_SIZE:
            raise IOError(
                "Corrupt video file: file is smaller than its footer."
            )
        stream.seek(file_size - 5)
        trailer = stream.read(5)
        if len(trailer) != 5:
            raise IOError("Corrupt video file: cannot read footer trailer.")
        magic, version = struct.unpack('<IB', trailer)
        if magic != self.MAGIC_NUMBER:
            raise IOError(
                "Corrupt video file: invalid footer magic %s." % magic
            )
        if version not in (self.V1_VERSION, self.VERSION):
            raise IOError("Unsupported video format version: %s" % version)

        footer_size = (
            self.V1_FOOTER_SIZE
            if version == self.V1_VERSION
            else self.FOOTER_SIZE
        )
        ...
        if version == self.V1_VERSION:
            ...
            index_names = (
                "physical video", "frame mapping", "run length",
        ...
        if version == self.V1_VERSION:
            ...
            frame_mapping_lengths = [0] * len(physical_lengths)
        else:
            (
                physical_lengths,
                frame_mapping_lengths,
                run_lengths,
                references,
                first_frames,
            ) = indexes
            if len(frame_mapping_lengths) != len(physical_lengths):
                raise IOError(
                    "Corrupt video file: physical video and frame mapping "
                    "indexes have different counts."
                )
            if any(length < 0 for length in frame_mapping_lengths):
                raise IOError(
                    "Corrupt video file: negative frame mapping length."
                )

        frame_mapping_size = sum(frame_mapping_lengths)
        mapping_start = index_start - frame_mapping_size
        if mapping_start < 0:
            raise IOError(
                "Corrupt video file: frame mappings exceed the file size."
            )
        physical_offsets = []
        payload_offset = 0
        ...
        frame_mapping_offsets = []
        mapping_offset = mapping_start
        for length in frame_mapping_lengths:
            frame_mapping_offsets.append(mapping_offset)
        ...
        self.frame_mapping_lengths = frame_mapping_lengths
        self.frame_mapping_offsets = frame_mapping_offsets
    ...
    def frame(self, returned_row: int):
        ...
        if self.frame_mapping_lengths[reference] == 0:
            return frame
        return frame + (
            self.frame_mapping_offsets[reference],
            self.frame_mapping_lengths[reference],
        )
