...
class DataWriter(ABC):
    ...
    def _write_data_to_file(self, data: pa.Table):
        ...
        if value_stats_enabled:
            if self.table.is_primary_key_table:
                ...
                value_fields = PyarrowFieldParser.to_paimon_schema(data.schema)
                value_stats_cols = None if self.write_cols is None else [field.name for field in value_fields]
        else:
            value_fields = []
            value_stats_cols = []
    ...
    def _get_column_stats(record_batch: pa.RecordBatch, column_name: str,
        ...
        supports_minmax = not (pa.types.is_nested(column_type) or pa.types.is_map(column_type)
                               ...
                               or (pa.types.is_decimal(column_type) and column_type.precision > 18)
                               or (pa.types.is_timestamp(column_type)
                                   and (column_type.tz is not None or column_type.unit not in ('s', 'ms'))))
