...
def validate_act_batch(batch, config):
    ...
    and ``sample_id == episode_id#frame_index`` identity.
    """
    required = {
    ...
    expected = {
        ...
        "frame_index": ((batch_size,), torch.int64),
    }
    ...
    for sample_id, episode_id, frame_index in zip(
            batch["sample_id"], batch["episode_id"],
            batch["frame_index"].tolist()):
        if sample_id != "%s#%s" % (episode_id, frame_index):
            raise ValueError(
                "sample_id is not aligned with episode_id and frame_index.")
