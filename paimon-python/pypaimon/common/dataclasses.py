...
# Python 3.6 compatibility: GenericAlias was introduced in Python 3.9
try:
    from types import GenericAlias
except ImportError:
    # For Python < 3.9, create a mock GenericAlias
    class GenericAlias:
        """Mock GenericAlias class for Python < 3.9 compatibility"""
...
# Helper function to safely check for GenericAlias
def _is_generic_alias(obj):
    """Safely check if obj is a GenericAlias, compatible with Python 3.6+"""
    try:
...
# Python 3.6 compatibility: typing._GenericAlias doesn't exist in Python 3.6
def _is_typing_generic_alias(obj):
    """Check if obj is typing._GenericAlias, compatible with Python 3.6+"""
    typing_module = sys.modules.get('typing')
    if typing_module and hasattr(typing_module, '_GenericAlias'):
        return type(obj) is typing_module._GenericAlias
    return False


# Python 3.6 compatibility: abc.update_abstractmethods was introduced in Python 3.7
def _update_abstractmethods(cls):
    """Update abstract methods, compatible with Python 3.6+"""
    if hasattr(abc, 'update_abstractmethods'):
        abc.update_abstractmethods(cls)
    else:
        # Fallback implementation for Python 3.6
        # This manually updates __abstractmethods__ based on the MRO
        abstracts = set()
...
class InitVar:
    ...
    def __repr__(self):
        if isinstance(self.type, type) and not _is_generic_alias(self.type):
...
class Field:
    ...
    # Python 3.6 compatibility: Only set __class_getitem__ if GenericAlias is available
    try:
...
def _repr_fn(fields, globals):
    # Create a straightforward repr function that's Python 3.6 compatible
    field_parts = []
    for f in fields:
        field_parts.append(f'f"{f.name}={{self.{f.name}!r}}"')

    if field_parts:
        fields_str = ' + ", " + '.join(field_parts)
        body = [f'return self.__class__.__qualname__ + "(" + {fields_str} + ")"']
    else:
...
def _is_classvar(a_type, typing):
    ...
    return (a_type is typing.ClassVar
            or (_is_typing_generic_alias(a_type)
...
def is_dataclass(obj):
    ...
    dataclass."""
    cls = obj if isinstance(obj, type) and not _is_generic_alias(obj) else type(obj)
