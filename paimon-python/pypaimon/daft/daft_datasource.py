...
class PaimonDataSource(DataSource):
    ...
    def _split_has_auth(split) -> bool:
        ...
        if isinstance(split, QueryAuthSplit):
            return True
        # ExplainSplitInfo (and any future descriptor) carries has_auth
        # so explain_scan can route consistently with the real read path.
        return bool(getattr(split, "has_auth", False))
