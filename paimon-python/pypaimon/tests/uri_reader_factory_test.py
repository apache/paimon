...
class UriReaderFactoryTest(unittest.TestCase):
    ...
    def test_from_file_io_cache_does_not_pin_file_io(self):
        import gc
        import weakref
        ...
        file_io = TokenFileIO()
        factory = UriReaderFactory.from_file_io(file_io)
        factory.create("file:///tmp/blob.bin")
        factory.create("https://example.com/blob.bin")
        self.assertIs(factory, UriReaderFactory.from_file_io(file_io))
        self.assertEqual(factory.get_cache_size(), 1)
        file_io_ref = weakref.ref(file_io)
        del file_io
        gc.collect()
        self.assertIsNone(file_io_ref())

    def test_from_file_io_factory_is_pickleable(self):
        import pickle
        import weakref

        file_io = FileIO.get(self.temp_dir)
        factory = UriReaderFactory.from_file_io(file_io)
        factory.create(f"file://{self.temp_file}")
        factory.create("https://example.com/blob.bin")
        # The factory only weakly refs FileIO, so FileIO must be in the pickle
        # graph (as it is on a table / ResolvingFileIO).
        restored_file_io, restored = pickle.loads(pickle.dumps((file_io, factory)))
        self.assertEqual(restored.get_cache_size(), 0)
        self.assertIsInstance(restored._provided_file_io, weakref.ref)
        self.assertIs(restored._provided_file_io(), restored_file_io)
        reader = restored.create(f"file://{self.temp_file}")
        self.assertIsInstance(reader, FileUriReader)
        stream = reader.new_input_stream(self.temp_file)
        try:
            self.assertEqual(stream.read().decode('utf-8'), "test content")
        finally:
            stream.close()
        self.assertIsInstance(
            restored.create("https://example.com/blob.bin"), HttpUriReader)
