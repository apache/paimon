...
class ResolvingFileIOTest(unittest.TestCase):
    ...
    def test_pickleable_after_uri_reader_factory_materialized(self):
        import pickle
        ...
        try:
            factory = resolving.uri_reader_factory
            self.assertIsNotNone(factory)
            restored = pickle.loads(pickle.dumps(resolving))
            self.assertIsInstance(restored, ResolvingFileIO)
            self.assertIsNotNone(restored.uri_reader_factory)
            reader = restored.uri_reader_factory.create("https://example.com/blob.bin")
            self.assertEqual(type(reader).__name__, "HttpUriReader")
        finally:
            resolving.close()

    def test_table_pickleable_after_uri_reader_factory_materialized(self):
        import pickle
        ...
        try:
            ...
            catalog.create_database("db", ignore_if_exists=True)
            schema = Schema.from_pyarrow_schema(
                pa.schema([("id", pa.int64()), ("name", pa.string())]))
            catalog.create_table("db.t", schema, False)
            table = catalog.get_table("db.t")
            self.assertIsInstance(table.file_io, ResolvingFileIO)
            pickle.dumps(table)
            table.file_io.uri_reader_factory
            restored = pickle.loads(pickle.dumps(table))
            self.assertIsNotNone(restored)
            self.assertIsNotNone(restored.file_io.uri_reader_factory)
            reader = restored.file_io.uri_reader_factory.create(
