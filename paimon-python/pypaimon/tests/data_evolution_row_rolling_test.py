...
class DataEvolutionRowRollingTest(unittest.TestCase):
    ...
    def _write_episode_files(self, table, data, episode_lengths):
        ...
        for length in episode_lengths:
            writer.roll_before_group_if_needed(length)
    ...
    def test_vector_rolling_waits_for_video_episode_boundary(self):
        ...
        for episode_lengths, expected_rows in (
                ([2, 2], [2, 2]), ([4], [4])):
            with self.subTest(episode_lengths=episode_lengths):
                table = self._create_with_schema(
                ...
                files = self._write_episode_files(
                    table, rows, episode_lengths)
                ...
                self.assertEqual(expected_rows, normal_rows)
                self.assertEqual(expected_rows, video_rows)
                self.assertEqual(expected_rows, vector_rows)
                self.assertEqual(list(range(4)), self._read_ids(table))
