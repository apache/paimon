...
class DataEvolutionChunkShuffleEndToEndTest(unittest.TestCase):
    ...
    def _commit_full_rows(self, table, pa_schema, ids):
        ...
        tc = wb.new_commit()
        tw.write_arrow(pa.Table.from_pydict(
            {'id': ids, 'value': [f'v{i}' for i in ids]}, schema=pa_schema))
        tc.commit(tw.prepare_commit())
        tw.close()
        tc.close()
    ...
    def test_workers_union_equals_full_table(self):
        ...
        # 4 commits → 4 aligned groups, one file each (full-column writes).
        for c in range(4):
            base = c * 50
            self._commit_full_rows(table, pa_schema, list(range(base, base + 50)))
    ...
    def test_deterministic_plan_across_calls(self):
        ...
        for c in range(3):
            base = c * 40
            self._commit_full_rows(table, pa_schema, list(range(base, base + 40)))
