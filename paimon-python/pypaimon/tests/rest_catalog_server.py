...
from ..api import RESTUtil
from ..api.api_response import (ConfigResponse, ListDatabasesResponse, GetDatabaseResponse,
                                GetTableResponse, ListTablesResponse,
                                RESTResponse, PagedList, ErrorResponse)
from ..api.resource_paths import ResourcePaths

from ..api.rest_json import JSON
from ..api.schema import Schema
from ..api.table_schema import TableSchema
from ..pynative.catalog.catalog_exception import DatabaseNotExistException, TableNotExistException, \
    DatabaseNoPermissionException, TableNoPermissionException
...
class RESTCatalogServer:
    ...
    def _route_request(self, method: str, resource_path: str, parameters: Dict[str, str],
        ...
        try:
            ...
            if resource_path.startswith(self.resource_paths.config()):
                warehouse_param = parameters.get(CoreOptions.WAREHOUSE)
    ...
    def _table_handle(self, method: str, data: str, identifier: Identifier) -> Tuple[str, int]:
        ...
        if method == "GET":
            if identifier.is_system_table():
                # Handle system table
                schema = Schema(fields=[], options={CoreOptions.PATH: f"/tmp/{identifier.get_full_name()}"})
            ...
            path = schema.options.pop(CoreOptions.PATH, None)
    ...
    def _generate_final_list_databases_response(self, parameters: Dict[str, str],
        ...
        if databases:
            ...
            response = ListDatabasesResponse(databases=[], next_page_token="")
