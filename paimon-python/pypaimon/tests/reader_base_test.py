...
class ReaderBasicTest(unittest.TestCase):
    ...
    def test_full_data_types(self):
        ...
        # binary columns (f8, f9) don't have min/max stats, aligned with Java behavior
        expected_min_values[8] = None
        expected_min_values[9] = None
        expected_max_values[8] = None
        expected_max_values[9] = None
    ...
    def test_metadata_stats_mode_defaults(self):
        ...
        self.assertEqual(core_options.metadata_stats_mode(), 'none')
        self.assertFalse(core_options.metadata_stats_enabled())
    ...
    def test_high_precision_timestamp_stats_skip_minmax(self):
        ...
        data = pa.Table.from_pydict(
            {
                'ts_us': [datetime(2024, 1, 1, 0, 0, 0, 999)],
                'ts_ns': [datetime(2024, 1, 1, 0, 0, 0, 999)],
            },
            schema=pa.schema([
        ...
        for column in ['ts_us', 'ts_ns']:
            stats = DataWriter._get_column_stats(data, column, 'full')
    ...
    def test_default_truncate_stats_e2e(self):
        ...
        pa_schema = pa.schema([('id', pa.int64()), ('name', pa.string())])
        schema = Schema.from_pyarrow_schema(pa_schema, options={'metadata.stats-mode': 'truncate(16)'})
    ...
    def test_default_truncate_binary_stats_e2e(self):
        ...
        min_row = GenericRowDeserializer.from_bytes(stats.min_values.data, table.fields)
        max_row = GenericRowDeserializer.from_bytes(stats.max_values.data, table.fields)
        self.assertIsNone(min_row.values[1])
        self.assertIsNone(max_row.values[1])

    def test_default_stats_skips_high_precision_decimal_minmax(self):
        catalog = CatalogFactory.create({"warehouse": self.warehouse})
        ...
        self.assertIsNone(min_row.values[1])
        self.assertIsNone(max_row.values[1])
        self.assertEqual(stats.null_counts, [0, 0])
        self.assertEqual(rb.new_read().to_arrow(scan.plan().splits()), data)

    def test_default_stats_with_high_precision_timestamp_e2e(self):
        catalog = CatalogFactory.create({"warehouse": self.warehouse})
        catalog.create_database("test_db_timestamp_stats", True)

        pa_schema = pa.schema([('id', pa.int64()), ('ts', pa.timestamp('us'))])
        schema = Schema.from_pyarrow_schema(pa_schema, options={'metadata.stats-mode': 'truncate(16)'})
        catalog.create_table("test_db_timestamp_stats.t", schema, False)
        table = catalog.get_table("test_db_timestamp_stats.t")

        value = datetime(2024, 1, 1, 0, 0, 0, 999)
        data = pa.Table.from_pydict({'id': [1], 'ts': [value]}, schema=pa_schema)
        wb = table.new_batch_write_builder()
        tw = wb.new_write()
        tc = wb.new_commit()
        tw.write_arrow(data)
    ...
    def test_primary_key_partial_write_value_stats(self):
        pa_schema = pa.schema([
            ('id', pa.int64()),
            ('name', pa.string()),
            ('price', pa.float64()),
        ...
        self.catalog.create_table('default.test_pk_partial_value_stats', schema, False)
        table = self.catalog.get_table('default.test_pk_partial_value_stats')
        ...
        write_builder = table.new_batch_write_builder()
        writer = write_builder.new_write().with_write_type(['name', 'id'])
        writer.write_arrow(partial_data)
        commit_messages = writer.prepare_commit()
        commit = write_builder.new_commit()
        commit.commit(commit_messages)
        writer.close()
        ...
        files = [file for msg in commit_messages for file in msg.new_files]
        self.assertGreater(len(files), 0)
        for file in files:
            self.assertIsNone(file.write_cols)
            self.assertEqual(file.value_stats_cols, ['id', 'name'])
            self.assertEqual(len(file.value_stats.min_values), 2)
            self.assertEqual(len(file.value_stats.max_values), 2)
            self.assertEqual(len(file.value_stats.null_counts), 2)
        ...
        entries = scan.file_scanner.manifest_file_manager.read(
            mf[0].file_name,
            lambda row: scan.file_scanner._filter_manifest_entry(row),
            False
        )
        stats_file = entries[0].file
        self.assertIsNone(stats_file.write_cols)
        self.assertEqual(stats_file.value_stats_cols, ['id', 'name'])
        stats_fields = [table.field_dict[col] for col in stats_file.value_stats_cols]
        min_row = GenericRowDeserializer.from_bytes(stats_file.value_stats.min_values.data, stats_fields)
        max_row = GenericRowDeserializer.from_bytes(stats_file.value_stats.max_values.data, stats_fields)
        self.assertEqual(min_row.values, [1, 'Alice'])
        self.assertEqual(max_row.values, [2, 'Bob'])

        actual = read_builder.new_read().to_arrow(scan.plan().splits())
