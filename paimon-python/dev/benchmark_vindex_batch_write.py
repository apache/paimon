...
"""Measure vector batch conversion/writing and native builds in fresh processes.

Run from paimon-python with the project dependencies installed::

    PYTHONPATH=. python dev/benchmark_vindex_batch_write.py prepare \
        --warehouse /tmp/paimon-batch-bench --rows 65536 --dimension 256
    PYTHONPATH=. python dev/benchmark_vindex_batch_write.py run \
        --warehouse /tmp/paimon-batch-bench --mode batch --batch-size 1024

Repeat run in separate processes for baseline, scalar-batches, convert-only, batch.
All modes retain whole-shard Arrow reading and the original training sampler.
scalar-batches converts one batch at a time to Python lists; convert-only uses
columnar conversion but writes each row separately; batch is the production path.
...
def scalar_build(per_batch):
    # Both reference modes retain the whole Arrow table through native finish,
    # just like master. Only the lifetime of Python row lists differs.
    def build(self, splits, ranges, field, table_read, index_path):
        ...
        for split, row_range in build_module._split_by_global_index_shard(
            ...
            try:
                batches = table.to_batches(max_chunksize=build_module.ADD_BATCH_SIZE) \
                    if per_batch else [table]
                for batch in batches:
                    for value, row_id in build_module._extract_index_rows(
...
class RowWrites:
    """Ablate batching of file writes while preserving columnar conversion.

    These are buffered Python file writes, not a count of OS write syscalls.
    """

    def __init__(self, stream, row_bytes):
        self.stream = stream
    ...
    def write(self, data):
        ...
        written = 0
        for start in range(0, len(data), self.row_bytes):
            written += self.stream.write(data[start:start + self.row_bytes])
        return written
...
def run(args):
    ...
    with ExitStack() as stack:
        stack.enter_context(patch.object(build_module, 'ADD_BATCH_SIZE', args.batch_size))
        if args.mode in ('baseline', 'scalar-batches'):
            stack.enter_context(patch.object(
                GlobalIndexBuilder, '_build_generic_index',
                scalar_build(args.mode == 'scalar-batches')))
        if args.mode == 'convert-only':
            original_ensure = VindexVectorIndexWriter._ensure_temp_files

            def ensure(writer):
                original_ensure(writer)
                if not isinstance(writer._row_id_temp, RowWrites):
                    writer._row_id_temp = RowWrites(writer._row_id_temp, 8)
                    writer._vector_temp = RowWrites(writer._vector_temp, dimension * 4)

            stack.enter_context(patch.object(VindexVectorIndexWriter, '_ensure_temp_files', ensure))
...
def main():
    ...
    parser.add_argument('--mode', choices=['baseline', 'scalar-batches', 'convert-only', 'batch'],
                        default='batch')
