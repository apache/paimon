...
/**
 * {@code MinioTestContainer} provides a {@code Minio} test instance for {@code paimon-s3-impl}.
 *
 * <p>This is a copy of the fixture in {@code paimon-s3}; it is duplicated here because {@code
 * paimon-s3-impl} is an upstream module and cannot depend on {@code paimon-s3}'s test-jar.
 */
public class MinioTestContainer extends GenericContainer<MinioTestContainer>
    ...
    /** Paimon S3 options wired to this container, including the root access/secret key. */
    ...
    /**
     * Paimon S3 options carrying temporary credentials (access key, secret key and session token)
     * issued by MinIO's STS endpoint via {@code AssumeRole}. Intended to exercise the S3A {@code
