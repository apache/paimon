...
public class CatalogContextFactoryTest {
    ...
    @Test
    public void testDetectionUtilityMethods() {
        // Test metastore detection
        Options hiveOptions = new Options();
        hiveOptions.set(METASTORE, "hive");
        assertThat(CatalogHadoopContext.needsHadoopForMetastore(hiveOptions)).isTrue();

        Options nonHiveOptions = new Options();
        nonHiveOptions.set(METASTORE, "filesystem");
        assertThat(CatalogHadoopContext.needsHadoopForMetastore(nonHiveOptions)).isFalse();

        // Test filesystem detection
        Options hdfsOptions = new Options();
        hdfsOptions.set(WAREHOUSE, "hdfs://namenode:8020/path");
        assertThat(CatalogHadoopContext.needsHadoopForFilesystem(hdfsOptions)).isTrue();

        Options s3Options = new Options();
        s3Options.set(WAREHOUSE, "s3://bucket/path");
        assertThat(CatalogHadoopContext.needsHadoopForFilesystem(s3Options)).isFalse();

        // Test security detection
        Options kerberosOptions = new Options();
        kerberosOptions.set("security.kerberos.login.principal", "user@REALM");
        assertThat(CatalogHadoopContext.needsHadoopForSecurity(kerberosOptions)).isTrue();

        Options noSecurityOptions = new Options();
        assertThat(CatalogHadoopContext.needsHadoopForSecurity(noSecurityOptions)).isFalse();
    }
