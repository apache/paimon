...
public class WeekdayTransform extends DateExtractTransform {
    ...
    @JsonCreator
    public WeekdayTransform(@JsonProperty(DateExtractTransform.FIELD_FIELD_REF) FieldRef fieldRef) {
        super(fieldRef);
    }

    public static Optional<Transform> tryCreate(FieldRef fieldRef) {
        return DateExtractTransform.tryCreate(fieldRef, WeekdayTransform::new);
    }
    ...
    protected Transform copy(FieldRef fieldRef) {
        return new WeekdayTransform(fieldRef);
    }
}
