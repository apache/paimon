...
public class DayOfWeekTransform extends DateExtractTransform {
    ...
    public static final String NAME = "DAY_OF_WEEK";
    ...
    public static Optional<Transform> tryCreate(FieldRef fieldRef) {
        return DateExtractTransform.tryCreate(fieldRef, DayOfWeekTransform::new);
    }
    ...
    protected Integer extract(LocalDateTime dateTime) {
        return dateTime.getDayOfWeek().getValue() % 7 + 1;
    }

    @Override
    protected Transform copy(FieldRef fieldRef) {
        return new DayOfWeekTransform(fieldRef);
    }
}
