...
public class PredicateJsonSerde {
    ...
    private static ObjectNode transformPredicateJsonNode(
        ...
        ArrayNode literalsNode = MAPPER.createArrayNode();
        if (literals != null) {
            DataType literalType = transform.outputType();
            for (Object literal : literals) {
                Object serializable = convertToSerializable(literalType, literal);
                literalsNode.add(MAPPER.valueToTree(serializable));
            }
        }
        predicate.set(FIELD_LITERALS, literalsNode);
        return predicate;
    }
    ...
    private static Object convertToSerializable(DataType literalType, Object literal) {
        if (literal == null) {
            return null;
        }
        switch (literalType.getTypeRoot()) {
            ...
            case DOUBLE:
                return literal;
            case DATE:
                return LocalDate.ofEpochDay((Integer) literal).toString();
            case TIME_WITHOUT_TIME_ZONE:
                return LocalTime.ofNanoOfDay(((Integer) literal) * 1_000_000L).toString();
            default:
