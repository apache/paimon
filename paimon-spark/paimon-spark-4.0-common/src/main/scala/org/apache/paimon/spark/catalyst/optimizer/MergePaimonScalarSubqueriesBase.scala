...
import org.apache.spark.sql.catalyst.expressions.{Alias, Attribute, AttributeMap, AttributeReference, CreateNamedStruct, ExprId, Expression, GetStructField, LeafExpression, Literal, NamedExpression, PredicateHelper, ScalarSubquery, Unevaluable}
...
trait MergePaimonScalarSubqueriesBase extends Rule[LogicalPlan] with PredicateHelper {
      ...
      .orElse((newPlan, cachedPlan) match {
        ...
        case (np, cp: Project) =>
          tryMergePlans(np, cp.child).map {
            case (mergedChild, outputMap) =>
              val (mergedProjectList, newOutputMap) =
                mergeNamedExpressions(np.output.asInstanceOf[Seq[NamedExpression]], outputMap, cp.projectList)
              val mergedPlan = Project(mergedProjectList, mergedChild)
              mergedPlan -> newOutputMap
          }
        case (np: Project, cp) =>
          tryMergePlans(np.child, cp).map {
            case (mergedChild, outputMap) =>
              val (mergedProjectList, newOutputMap) =
                mergeNamedExpressions(np.projectList, outputMap, cp.output.asInstanceOf[Seq[NamedExpression]])
              val mergedPlan = Project(mergedProjectList, mergedChild)
              mergedPlan -> newOutputMap
          }
  ...
  private def supportedAggregateMerge(newPlan: Aggregate, cachedPlan: Aggregate) = {
    ...
    val newPlanSupportsHashAggregate = Aggregate.supportsHashAggregate(
      newPlanAggregateExpressions.flatMap(_.aggregateFunction.aggBufferAttributes:Iterable[AttributeReference]),newPlan.groupingExpressions)
    val cachedPlanSupportsHashAggregate = Aggregate.supportsHashAggregate(
      cachedPlanAggregateExpressions.flatMap(_.aggregateFunction.aggBufferAttributes:Iterable[AttributeReference]),cachedPlan.groupingExpressions)
