...
import org.apache.spark.sql.catalyst.expressions.{Alias, Attribute, AttributeSeq, Cast, Expression, NamedExpression}
...
class PaimonAnalysis(session: SparkSession) extends Rule[LogicalPlan] {
  ...
  override def apply(plan: LogicalPlan): LogicalPlan = plan.resolveOperatorsDown {
    ...
    case a @ PaimonV2WriteCommand(table, paimonTable)
        if !schemaCompatible(
          new AttributeSeq(a.query.output.toIndexedSeq).toStructType,
          table.output.toStructType,
          paimonTable.partitionKeys().asScala.toSeq) =>
  ...
  private def resolveQueryColumns(
    ...
    Project(project.toIndexedSeq, query)
  }
  ...
  private def checkNullability(
    ...
    if (!partitionCols.contains(fullColumnName) && input.nullable && !expected.nullable) {
      throw new RuntimeException("Cannot write nullable values to non-null column (  input  "+input.name+" , expected "+expected.name+")")
    }
  }
}
