...
case class PaimonCreateTableAsSelectStrategy(spark: SparkSession)
  ...
  private def tableSpecWithOptions(
      tableSpec: TableSpec, options: Map[String, String]): (TableSpec, Map[String, String]) = {
  ...
  private def failIfPartitionedFormatTable(
      catalog: SparkCatalog, tableSpec: TableSpec, parts: Seq[_]): Unit = {
  ...
  private def invalidateCache(
      catalog: TableCatalog, table: Table, ident: Identifier): Unit = {
    val v2Relation = DataSourceV2Relation.create(table, Some(catalog), Some(ident))
    val classicSpark = spark.asInstanceOf[org.apache.spark.sql.classic.SparkSession]
    classicSpark.sharedState.cacheManager.uncacheQuery(classicSpark, v2Relation, true, false)
  }
}
