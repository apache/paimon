...
class FormatTablePartitionManagementTest extends SparkFunSuite {
  ...
  test("managed ADD forwards the complete batch and ignoreIfExists to the partition catalog") {
    ...
    val sparkTable = new PaimonFormatTable(managedFormatTable(), gateway)
  ...
  test("managed ADD creates the partition directory") {
    val fileIO = LocalFileIO.create()
    val tablePath = new Path(Files.createTempDirectory("managed-format-add-mkdirs").toUri)
    val table = managedFormatTable(fileIO, tablePath.toString)
    val partitionDir = new Path(tablePath, "dt=20260715/hh=10")
    try {
      val sparkTable = new PaimonFormatTable(table, emptyGateway)
  ...
  test("managed DROP rejects a partition value that would escape the table location") {
    var dropCalls = 0
    val gateway = new FormatTablePartitionCatalog {
      override def createPartitions(
          partitions: JList[JMap[String, String]],
    ...
    val sparkTable =
      new PaimonFormatTable(managedStringFormatTable(onlyValueInPath = true), gateway)
  ...
  test("managed ADD with LOCATION is rejected before any catalog RPC") {
    var createCalls = 0
    val gateway = new FormatTablePartitionCatalog {
      override def createPartitions(
          partitions: JList[JMap[String, String]],
  ...
  test("managed DROP PARTITION unregisters first and then deletes the partition directory") {
    var dropped = Seq.empty[JMap[String, String]]
    var directoryExistedAtUnregister = false
    val fileIO = LocalFileIO.create()
    val tablePath = new Path(Files.createTempDirectory("managed-format-drop-ordering").toUri)
    val table = managedFormatTable(fileIO, tablePath.toString)
    val partitionDir = new Path(tablePath, "dt=20260715/hh=10")
    ...
    try {
      ...
      val sparkTable = new PaimonFormatTable(table, gateway)
      assert(
        sparkTable.dropFormatTablePartitions(
          Array(Array("dt", "hh")),
          Array(new GenericInternalRow(Array[Any](20260715, 10)))))
  ...
  test("managed direct partial DROP expands leading values to exact leaf partitions") {
    val fileIO = LocalFileIO.create()
    val tablePath = new Path(Files.createTempDirectory("managed-format-direct-partial-drop").toUri)
    ...
    val gateway = new FormatTablePartitionCatalog {
      ...
      override def listPartitions(
        ...
        FormatTablePartitionPage(Seq(first.asJava, second.asJava).asJava, null)
      }
    }
    ...
    try {
      ...
      val sparkTable =
        new PaimonFormatTable(managedFormatTable(fileIO, tablePath.toString), gateway)
  ...
  test("managed DROP fails when FileIO reports an undeleted partition directory") {
    val delegate = LocalFileIO.create()
    val tablePath = new Path(Files.createTempDirectory("managed-format-drop").toUri)
    ...
    try {
      val error = intercept[java.io.IOException] {
        new PaimonFormatTable(table, gateway)
  ...
  test("managed DROP treats an already missing partition directory as deleted") {
    val delegate = LocalFileIO.create()
    val tablePath = new Path(Files.createTempDirectory("managed-format-drop-missing").toUri)
    val fileIO = falseDeleteFileIO(delegate)
    val table = managedFormatTable(fileIO, tablePath.toString)
    val gateway = new RecordingDropCatalog
  ...
  test("managed DROP deduplicates overlapping full and partial specs before deleting") {
    val delegate = LocalFileIO.create()
    val tablePath = new Path(Files.createTempDirectory("managed-format-drop-overlap").toUri)
    val partitionDir = new Path(tablePath, "dt=20260715/hh=10")
    var deleteCalls = 0
    val fileIO = Proxy
      .newProxyInstance(
  ...
  test("managed partial DROP leaves catalog and data untouched when a later page fails") {
    val fileIO = LocalFileIO.create()
    val tablePath = new Path(Files.createTempDirectory("managed-format-drop-page-failure").toUri)
    val partitionDir = new Path(tablePath, "dt=20260715/hh=10")
    val matching = partitionSpec(20260715, 10)
    var dropCalls = 0
    val gateway = new FormatTablePartitionCatalog {
      ...
      override def listPartitions(
        ...
        Option(pageToken) match {
          case None => FormatTablePartitionPage(Seq(matching.asJava).asJava, "page-2")
          case Some("page-2") => throw new TestPartitionPageException
    ...
    try {
      ...
      intercept[TestPartitionPageException] {
        sparkTable.dropFormatTablePartitions(
          Array(Array("hh")),
  ...
  test("managed partial DROP keeps data untouched after an ambiguous catalog response") {
    val fileIO = LocalFileIO.create()
    val tablePath = new Path(Files.createTempDirectory("managed-format-drop-response-loss").toUri)
    ...
    val gateway = new FormatTablePartitionCatalog {
      ...
      override def listPartitions(
          ...
          pageToken: String,
          pageSize: Int): FormatTablePartitionPage =
        FormatTablePartitionPage(registered.toSeq.map(_.asJava).asJava, null)
    }
  ...
  test("managed IF NOT EXISTS delegates all and partial duplicate batches to the catalog") {
    val first = partitionSpec(20260715, 10)
    val second = partitionSpec(20260716, 11)
    val third = partitionSpec(20260717, 12)
    val gateway = new InMemoryPartitionCatalog(Seq(first, second))
    val sparkTable = new PaimonFormatTable(managedFormatTable(), gateway)
    ...
    assert(gateway.partitions == Seq(first, second, third))
    assert(gateway.lookupCalls == 0)
  }

  test("managed ordinary ADD propagates the catalog duplicate error") {
    val existing = partitionSpec(20260715, 10)
    val gateway = new InMemoryPartitionCatalog(Seq(existing))
    val sparkTable = new PaimonFormatTable(managedFormatTable(), gateway)
  ...
  test("concurrent managed IF NOT EXISTS ADD relies on catalog atomicity") {
    val first = partitionSpec(20260715, 10)
    val second = partitionSpec(20260716, 11)
    val third = partitionSpec(20260717, 12)
    val gateway = new InMemoryPartitionCatalog
    val sparkTable = new PaimonFormatTable(managedFormatTable(), gateway)
    val start = new CountDownLatch(1)
    val done = new CountDownLatch(2)
    val failures = Collections.synchronizedList(new ArrayList[Throwable]())

    def add(rows: Array[InternalRow]): Thread =
      new Thread(new Runnable {
        override def run(): Unit = {
          try {
            ...
            sparkTable.createFormatTablePartitions(
              rows,
              Array.fill[JMap[String, String]](rows.length)(Collections.emptyMap()),
              ignoreIfExists = true)
          } catch {
    ...
    val left = add(Array(partitionRow(20260715, 10), partitionRow(20260716, 11)))
    val right = add(Array(partitionRow(20260716, 11), partitionRow(20260717, 12)))
    left.start()
    right.start()
    start.countDown()
    ...
    assert(gateway.partitions.toSet == Set(first, second, third))
    assert(gateway.createRequests.size == 2)
    assert(gateway.createRequests.forall(_._2))
    assert(gateway.lookupCalls == 0)
  }

  test("managed ADD preserves typed special partition values") {
    val gateway = new InMemoryPartitionCatalog
    val sparkTable = new PaimonFormatTable(managedStringFormatTable(), gateway)
  ...
  private def emptyGateway: FormatTablePartitionCatalog =
    new FormatTablePartitionCatalog {
      override def createPartitions(
          partitions: JList[JMap[String, String]],
          ignoreIfExists: Boolean): Unit = {}
  ...
  private class InMemoryPartitionCatalog(initialPartitions: Seq[Map[String, String]] = Seq.empty)
    ...
    override def listPartitionsByNames(
        partitions: JList[JMap[String, String]]): JList[JMap[String, String]] = synchronized {
      lookupCalls += 1
      partitions.asScala
        .map(_.asScala.toMap)
        .filter(storedPartitions.contains)
        .map(_.asJava)
        .asJava
    }
  ...
  private class TestPartitionPageException extends RuntimeException

  private class TestCatalogDropResponseException extends RuntimeException

  private class RecordingDropCatalog extends FormatTablePartitionCatalog {
    var dropCalls = 0
  ...
  private def managedFormatTable(
      fileIO: FileIO = LocalFileIO.create,
      location: String = uniqueTableLocation("format_table")): FormatTable = {
    FormatTable
      .builder()
      .fileIO(fileIO)
  ...
  private def managedStringFormatTable(onlyValueInPath: Boolean = false): FormatTable = {
    FormatTable
      .builder()
