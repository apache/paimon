...
class ParquetEncryptionTest extends PaimonSparkTestBase {
      ...
      withTable("t") {
        ...
        val filePath = new Path(
          sql("SELECT __paimon_file_path FROM t LIMIT 1")
            ...
            .as[String]
            .head)
        assertFooterEncryptionType(filePath, FileMetaData.EncryptionType.ENCRYPTED_FOOTER)
      }
    }
  }
  ...
  private def assertFooterEncryptionType(
      filePath: Path,
      expert: FileMetaData.EncryptionType): Unit = {
    val reader =
