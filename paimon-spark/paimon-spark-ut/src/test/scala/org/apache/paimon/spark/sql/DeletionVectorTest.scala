...
import org.apache.paimon.deletionvectors.{BucketedDvMaintainer, BucketedDvMaintainerTest, DeletionVector, FileNameKey}
    ...
    partitions.flatMap {
      partition =>
        BucketedDvMaintainerTest
          ...
          .deletionVectors()
          .asScala
          .map { case (key: FileNameKey, dv) => key.fileName() -> dv }
    }.toMap
  }
