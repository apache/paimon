...
class DeletionVectorTest extends PaimonSparkTestBase with AdaptiveSparkPlanHelper {
  ...
  test(s"Paimon DeletionVector: delete for append partitioned table with bucket = 1") {
    val bucket = -1
    withTable("T") {
      val bucketKey = if (bucket > 1) {
        ", 'bucket-key' = 'id'"
      ...
      def getDeletionVectors(ptValues: Seq[String]): Map[String, DeletionVector] = {
        getLatestDeletionVectors(table, dvMaintainerFactory, ptValues.map(BinaryRow.singleColumn))
      }
