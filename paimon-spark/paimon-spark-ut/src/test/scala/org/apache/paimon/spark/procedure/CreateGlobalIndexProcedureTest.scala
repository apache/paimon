...
class CreateGlobalIndexProcedureTest extends PaimonSparkTestBase with StreamTest {
  ...
  test("create faiss-vector-ann global index") {
    withTable("T") {
      spark.sql("""
  ...
  test("create faiss-vector-ann global index with partition") {
    withTable("T") {
      spark.sql("""
      ...
      assert(indexEntries.nonEmpty)
      val totalRowCount = indexEntries.map(_.indexFile().rowCount()).sum
      assert(totalRowCount == 1000L)
    }
  }

  test("create faiss-vector-ann global index with IVF index type") {
    withTable("T") {
      spark.sql("""
                  |CREATE TABLE T (id INT, v ARRAY<FLOAT>)
                  |TBLPROPERTIES (
      ...
      val output =
        spark
          .sql("CALL sys.create_global_index(table => 'test.T', index_column => 'v', index_type => 'faiss-vector-ann', options => 'vector.dim=3,vector.index-type=IVF,vector.nlist=10')")
          .collect()
          .head
