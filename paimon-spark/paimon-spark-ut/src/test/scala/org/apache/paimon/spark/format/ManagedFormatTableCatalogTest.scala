...
class ManagedFormatTableCatalogTest extends PaimonSparkTestWithRestCatalogBase {

  test("SparkCatalog injects a branch-aware partition catalog into a managed Format Table") {
    val tableName = "managed_format_table_catalog_gateway"
    withTable(tableName) {
      sql(
      ...
      assert(sparkTable.partitionCatalog != null)
    }
  }
}
