...
object PaimonTableValuedFunctions {
   ...
   * push into Paimon, when `spark.paimon.search.residual-filter` is `fail`. Such a residual is
   * applied by Spark above the search, whose result is already truncated to the top-K, so a row
   * that satisfies the residual but ranks just outside the returned K is lost and the result comes
   * back short. Failing is consistent with the Flink `vector_search` procedure, which rejects an
   * inexpressible predicate rather than silently dropping rows.
   */
  def failUnpushableSearchFilter(residuals: Seq[String]): Nothing = {
    throw new UnsupportedOperationException(
      "Vector, hybrid or full-text search does not support a filter that cannot be pushed down " +
        ...
        "comparisons and unresolvable casts), or set spark.paimon.search.residual-filter to " +
        "post-filter to apply it above the search and accept a possibly short result.")
  }
