...
trait RowLevelHelper extends SQLConfHelper {
  ...
  protected def shouldFallbackToV1MergeInto(m: MergeIntoTable): Boolean = {
    ...
    shouldFallbackToV1(table) ||
    // Data-evolution tables expose V2 row-level ops for UPDATE only; MERGE keeps the V1
    // `MergeIntoPaimonDataEvolutionTable` command which writes partial-column patch files instead
    // of rewriting whole rows.
