...
object PaimonWriteRequirement {
  ...
  /**
   * Requirement for copy-on-write UPDATE on data-evolution tables: range-distribute and sort by
   * `_ROW_ID` so each task receives contiguous row-id segments and the writer can emit files whose
   * row ids derive from `firstRowId` plus position. `_ROW_ID` is preserved for both carryover and
   * updated rows, which also keeps rows of one partition together (row id ranges never span
   * partitions).
   */
