 ...
 * drop rows and never refill the ones displaced out of the top-K. Only active when
 * `spark.paimon.search.residual-filter` is `fail`; the default `post-filter` keeps the residual and
 * accepts a possibly short result. The lateral form is handled by
