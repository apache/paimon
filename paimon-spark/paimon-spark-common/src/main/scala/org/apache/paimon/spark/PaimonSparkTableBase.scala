...
import org.apache.paimon.utils.{ChainTableUtils, StringUtils}
  ...
  override def newWriteBuilder(info: LogicalWriteInfo): WriteBuilder = {
    table match {
      case fileStoreTable: FileStoreTable =>
        ...
        val writeTable = if (ChainTableUtils.isChainTable(fileStoreTable)) {
          ChainTableUtils.getWriteTable(fileStoreTable)
        } else {
          fileStoreTable
        }
        if (useV2Write) {
          new PaimonV2WriteBuilder(writeTable, info.schema(), options)
        } else {
