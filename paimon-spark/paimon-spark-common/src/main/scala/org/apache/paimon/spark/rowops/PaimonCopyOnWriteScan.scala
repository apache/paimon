...
case class PaimonCopyOnWriteScan(
  ...
  private def filterDataEvolutionGroups(split: DataSplit): Option[DataSplit] = {
    ...
    if (kept.isEmpty) {
      ...
      // The rebuild below copies only data files. Data-evolution tables never carry deletion
      // vectors (the V2 capability gate excludes DV), so dropping them here would be a silent
      // correctness bug rather than a supported case: fail loudly instead.
      assert(
        !split.deletionFiles().isPresent,
        s"Data-evolution copy-on-write split unexpectedly carries deletion files, this is a bug: $split")
