  ...
  private def normalizeCatalogManagedPartitionOptions(
    ...
    table match {
      case formatTable: FormatTable =>
        val partitionSourceKey = CoreOptions.FORMAT_TABLE_PARTITION_SOURCE.key()
        val persistedPartitionsFromCatalog =
          new CoreOptions(formatTable.options()).formatTablePartitionsFromCatalog()
        if (dynamicOptions.containsKey(partitionSourceKey)) {
          val dynamicPartitionsFromCatalog =
            new CoreOptions(dynamicOptions).formatTablePartitionsFromCatalog()
