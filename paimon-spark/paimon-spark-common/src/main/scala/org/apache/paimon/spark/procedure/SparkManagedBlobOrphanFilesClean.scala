...
case class SparkManagedBlobOrphanFilesClean(
  ...
  def doClean(): (Dataset[(Long, Long)], Seq[Dataset[_]]) = {
    ...
    try {
      ...
      val deleted: Dataset[(Long, Long)] = if (frozenAbort) {
        val reason =
          if (usedSetChanged) {
        ...
        Seq.empty[(Long, Long)].toDS()
      } else {
        // persist() does not cut sidecar lineage. Collect the validated used names onto a
        // driver-backed Dataset so the deletion join cannot recompute from stale sidecar shuffle
        // after a cache miss, drop a reused pack, and still agree.
        // Cost: driver memory grows with the used-pack set. An OOM fails the procedure instead of
        // deleting live packs. Do not join the persisted used2Packs Dataset again; that puts sidecar
        // reads back into the deletion lineage.
        val frozenUsedNames = used2Packs.collect().map(_.getString(0))
        logInfo(
          s"Frozen ${frozenUsedNames.length} used managed blob pack names for table ${table.fullName()} before deletion.")
        val frozenUsed = frozenUsedNames.toSeq.toDF("used_name")
        canonicalCandidates
          .join(frozenUsed, $"name" === $"used_name", "left_anti")
...
object SparkManagedBlobOrphanFilesClean extends SQLConfHelper {
  ...
  def executeDatabase(
    ...
    var deletedFilesCount = 0L
    var deletedFilesLenInBytes = 0L
    // Run one table at a time and unpersist its marks before the next. Unlike
    // SparkOrphanFilesClean, abort gating is per-table, so unioning every table's
    // deletion Dataset into one job would keep all mark caches alive for the whole
    // database and mix independent skip/used-set signals.
    tables.foreach {
      table =>
