...
trait PaimonPartitionManagement extends SupportsAtomicPartitionManagement with Logging {
  ...
  def partitionCatalog: FormatTablePartitionCatalog = null
   ...
   * specs are registered for a managed Format Table. The result is aligned with the input arrays.
   */
  private[spark] def formatTablePartitionsRegistered(
    ...
    table match {
      case formatTable: FormatTable
          if CoreOptions.fromMap(formatTable.options()).partitionedTableInMetastore() =>
        val requested =
          rows.zip(partitionNames).map { case (row, names) => toPaimonPartition(row, names.toSeq) }
        val registered = requirePartitionCatalog().listPartitionsByNames(requested.toSeq.asJava)
        val registeredSpecs = registered.asScala.map(_.asScala.toMap).toSet
        requested.map(spec => registeredSpecs.contains(spec.asScala.toMap))
      case _ =>
        throw new UnsupportedOperationException(
          "Only managed Format Table supports partition registration lookup.")
    }
  }
    ...
    table match {
      case formatTable: FormatTable
        ...
        if (partialSpecs.nonEmpty) {
          ...
          // One unfiltered paged traversal resolves every partial spec; the requested constraints
          // are enforced client-side.
          val catalog = requirePartitionCatalog()
          var pageToken: String = null
          do {
            val page = catalog.listPartitions(
              new java.util.LinkedHashMap[String, String](),
              pageToken,
              PaimonPartitionManagement.ManagedPartitionDropPageSize)
            page.partitions.asScala.foreach {
              partition =>
                val validated = validateManagedDropPartition(formatTable, partition)
        ...
        dropManagedFormatTablePartitions(formatTable, partitions.toSeq)
      case _ =>
        throw new UnsupportedOperationException(
          "Only managed Format Table supports named partition drop.")
    }
  }

  private def dropManagedFormatTablePartitions(
    ...
    logInfo("Try to drop managed format table partitions: " + partitions.mkString(","))
    requirePartitionCatalog().dropPartitions(partitions.asJava)
  ...
  private def validateManagedDropPartition(
      formatTable: FormatTable,
    ...
    if (partitionKeys.exists(key => partition.get(key) == null)) {
      throw new IllegalStateException(
        s"Catalog must return a complete partition spec with keys " +
          s"${partitionKeys.mkString("[", ", ", "]")} for managed Format Table " +
  ...
  private[spark] def createFormatTablePartitions(
    ...
    requirePartitionCatalog().createPartitions(specs.asJava, ignoreIfExists)
  ...
  private def requirePartitionCatalog(): FormatTablePartitionCatalog = {
    if (partitionCatalog == null) {
      throw new UnsupportedOperationException(
        s"Partition catalog is not configured for managed Format Table ${table.fullName()}.")
    }
    partitionCatalog
  }
}

private object PaimonPartitionManagement {
  val ManagedPartitionDropPageSize: Int = 1000
}
