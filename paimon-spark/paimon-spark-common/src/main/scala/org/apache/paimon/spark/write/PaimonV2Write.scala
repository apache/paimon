...
import org.apache.paimon.spark.{PaimonAppendedChangelogFilesMetric, PaimonAppendedRecordsMetric, PaimonAppendedTableFilesMetric, PaimonBucketsWrittenMetric, PaimonCommitDurationMetric, PaimonCopyOnWriteScan, PaimonNumWritersMetric, PaimonPartitionsWrittenMetric, SparkInternalRowWrapper, SparkUtils}
...
import org.apache.paimon.table.sink.{BatchWriteBuilder, CommitMessage, CommitMessageImpl, CommitMessageSerializer, TableWriteImpl}
  ...
  override def commit(messages: Array[WriterCommitMessage]): Unit = {
    ...
    try {
      if (scan.isEmpty) {
        ...
        val addCommitMessages = messages
          .collect {
            case taskCommit: TaskCommit => taskCommit.commitMessages()
            case other =>
              throw new IllegalArgumentException(s"${other.getClass.getName} is not supported")
