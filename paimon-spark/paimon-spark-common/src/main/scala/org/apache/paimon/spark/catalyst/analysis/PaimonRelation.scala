...
object PaimonRelation extends Logging {
  ...
  def unapply(plan: LogicalPlan): Option[PaimonTable] =
    EliminateSubqueryAliases(plan) match {
      case DataSourceV2Relation(table: PaimonTable, _, _, _, _) => Some(table)
      case ResolvedTable(_, _, table: PaimonTable, _) => Some(table)
      case _ => None
    }
  ...
  def getPaimonRelation(plan: LogicalPlan): DataSourceV2Relation = {
    EliminateSubqueryAliases(plan) match {
      case d @ DataSourceV2Relation(_: PaimonTable, _, _, _, _) => d
