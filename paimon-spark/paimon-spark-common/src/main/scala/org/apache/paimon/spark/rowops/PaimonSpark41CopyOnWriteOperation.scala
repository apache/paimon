...
class PaimonSpark41CopyOnWriteOperation(table: FileStoreTable, info: RowLevelOperationInfo)
  ...
  override def newScanBuilder(options: CaseInsensitiveStringMap): ScanBuilder = {
    new PaimonBaseScanBuilder {
      override lazy val table: FileStoreTable =
        PaimonSpark41CopyOnWriteOperation.this.table.copy(options.asCaseSensitiveMap)

      override def build(): Scan = {
        val scan =
          PaimonCopyOnWriteScan(table, requiredSchema, pushedPartitionFilters, pushedDataFilters)
        PaimonSpark41CopyOnWriteOperation.this.copyOnWriteScan = Option(scan)
        scan
      }
    }
  }
