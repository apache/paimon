...
case class RewriteSparkDDLCommands(spark: SparkSession)
  ...
  override def apply(plan: LogicalPlan): LogicalPlan = plan.resolveOperatorsUp {
    ...
    //
    // Both Paimon data tables and catalog-managed Format Tables route DROP PARTITION through
    // PaimonDropPartitions (managed Format Tables resolve partial specs through the partition
    // gateway, so they must bypass Spark's strict partition-spec resolution too). A single
    // extractor loads the table once and classifies it, avoiding a second catalog.loadTable per
    // analyzer iteration. Unmanaged Format Tables are not matched and keep Spark's own resolution
    // plus the explicit unsupported error at execution.
        ...
        Try(catalog.loadTable(ident))
          .map {
            ...
            case formatTable: PaimonFormatTable =>
              PaimonFormatTablePartitionDdlExec.isManaged(formatTable) &&
              formatTable.partitionCatalog != null
            case _ => false
          }
          .getOrElse(false)
      case _ => false
