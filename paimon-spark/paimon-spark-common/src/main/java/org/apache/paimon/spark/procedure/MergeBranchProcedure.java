...
public class MergeBranchProcedure extends BaseProcedure {
    ...
    public InternalRow[] call(InternalRow args) {
        ...
        String targetBranch = args.isNullAt(2) ? DEFAULT_MAIN_BRANCH : args.getString(2);
        InternalRow[] result =
                modifyPaimonTable(
                        tableIdent,
                        table -> {
        ...
        if (!DEFAULT_MAIN_BRANCH.equals(targetBranch)) {
            String branchTableName =
                    tableIdent.name() + SYSTEM_TABLE_SPLITTER + SYSTEM_BRANCH_PREFIX + targetBranch;
            Identifier branchIdent = Identifier.of(tableIdent.namespace(), branchTableName);
