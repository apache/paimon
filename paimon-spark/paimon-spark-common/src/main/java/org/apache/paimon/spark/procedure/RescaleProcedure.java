...
public class RescaleProcedure extends BaseProcedure {
    ...
    public InternalRow[] call(InternalRow args) {
        ...
        String partition = blank(args, 2) ? null : args.getString(2);
        Integer scanParallelism = args.isNullAt(3) ? null : args.getInt(3);
        Integer sinkParallelism = args.isNullAt(4) ? null : args.getInt(4);
                ...
                table -> {
                    ...
                    if (partition != null) {
                        List<Map<String, String>> partitions =
                                ParameterUtils.getPartitions(partition.split(";"));
                        checkArgument(
                                partitions.size() == 1,
                                "Rescale only supports one partition at a time, but got %d partitions",
                                partitions.size());
                        partitionMap = partitions.get(0);
                    }
    ...
    private void execute(
        ...
        if (partitionMap.isEmpty()) {
            ...
            writer.writeBuilder().withOverwrite(partitionMap);
        }
    ...
    private boolean blank(InternalRow args, int index) {
        return args.isNullAt(index)
                || (args.getString(index) == null || args.getString(index).trim().isEmpty());
    }

    @Override
    public String description() {
        return "This procedure rescales one partition of a table by changing the bucket number.";
    }
