 ...
 *
 * Row-tracking-only and data-evolution tables use the same V2 copy-on-write rewrite (the latter
 * additionally rejects partition-column updates). PK / DV tables go through the postHoc V1 rule
 * because they do not expose `SupportsRowLevelOperations`. DELETE is handled by
