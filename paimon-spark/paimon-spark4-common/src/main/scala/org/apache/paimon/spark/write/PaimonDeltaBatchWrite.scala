...
/**
 * Spark-4.x thin wrapper that mixes `DeltaBatchWrite` into [[PaimonDeltaWriteBase]]. See the base
 * class scaladoc for why the inheritance lives here rather than in `paimon-spark-common`; this
 * class is compiled against Spark 4.1 and shadowed in `paimon-spark-4.0/src/main` for the 4.0.2
 * compile target.
 */
