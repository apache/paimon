...
/**
 * Spark-4.x thin wrapper that mixes `BatchWrite` into [[FormatTableBatchWriteBase]]. See the base
 * class scaladoc for why the inheritance lives here rather than in `paimon-spark-common`. A
 * duplicate of this file lives at `paimon-spark-4.0/src/main` so that a 4.0.2 compile target
 * produces a class file whose method table does not reference `WriteSummary` (4.1-only).
 */
