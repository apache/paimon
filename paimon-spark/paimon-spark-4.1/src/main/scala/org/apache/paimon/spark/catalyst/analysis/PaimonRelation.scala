...
object PaimonRelation extends Logging {
  ...
  def unapply(plan: LogicalPlan): Option[SparkTable] =
    EliminateSubqueryAliases(plan) match {
      case Project(_, DataSourceV2Relation(table: SparkTable, _, _, _, _, _)) => Some(table)
      case DataSourceV2Relation(table: SparkTable, _, _, _, _, _) => Some(table)
  ...
  def getPaimonRelation(plan: LogicalPlan): DataSourceV2Relation = {
    EliminateSubqueryAliases(plan) match {
      case Project(_, d @ DataSourceV2Relation(_: SparkTable, _, _, _, _, _)) => d
      case d @ DataSourceV2Relation(_: SparkTable, _, _, _, _, _) => d
