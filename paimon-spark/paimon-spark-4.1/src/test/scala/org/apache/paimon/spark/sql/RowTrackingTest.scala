...
class RowTrackingTest extends RowTrackingTestBase {
  ...
  test("Data Evolution: Spark 4.1 uses V2 copy-on-write for DELETE and UPDATE") {
    withSparkSQLConf("spark.paimon.write.use-v2-write" -> "true") {
      withTable("t") {
        sql(
          "CREATE TABLE t (id INT, data INT) TBLPROPERTIES " +
            "('row-tracking.enabled' = 'true', 'data-evolution.enabled' = 'true')")
        // One file (= one row-id group) per INSERT, so the DML below rewrites groups that
        // still contain carryover rows.
        sql("INSERT INTO t SELECT /*+ REPARTITION(1) */ * FROM VALUES (1, 1), (2, 2)")
        sql("INSERT INTO t SELECT /*+ REPARTITION(1) */ * FROM VALUES (3, 3), (4, 4)")

        assertPlanContains("DELETE FROM t WHERE id = 2", "ReplaceData")
        sql("DELETE FROM t WHERE id = 2")
        // The touched row-id group [0, 1] is rewritten with preserved row ids and a fresh
        // sequence number (the new snapshot id); the untouched group [2, 3] keeps its file.
        checkAnswer(
          sql("SELECT *, _ROW_ID, _SEQUENCE_NUMBER FROM t ORDER BY id"),
          Seq(Row(1, 1, 0, 3), Row(3, 3, 2, 2), Row(4, 4, 3, 2))
        )
        ...
        checkAnswer(
          sql("SELECT *, _ROW_ID, _SEQUENCE_NUMBER FROM t ORDER BY id"),
          Seq(Row(1, 1, 0, 3), Row(3, 30, 2, 4), Row(4, 4, 3, 4))
        )
      }
    }
  }

  test("Data Evolution: V2 delete rewrites whole row-id groups with patch files") {
    withSparkSQLConf("spark.paimon.write.use-v2-write" -> "true") {
      withTable("s", "t") {
        sql(
        ...
        assert(filePaths.length == 2 && filePaths.forall(_ != null), filePaths.mkString(", "))
        ...
        sql("DELETE FROM t WHERE data = 20 AND name = 'b'")
        checkAnswer(
          sql("SELECT *, _ROW_ID FROM t ORDER BY id"),
          Seq(Row(3, 30, "c", 2))
        )
  ...
  test("Data Evolution: Spark 4.1 restores metadata-only delete fast path") {
    withSparkSQLConf("spark.paimon.write.use-v2-write" -> "true") {
      withTable("t") {
        sql("""
              |CREATE TABLE t (id INT, data INT, dt STRING)
        ...
        checkAnswer(sql("SELECT * FROM t ORDER BY id"), Seq(Row(3, 3, "p2")))
      }
    }
  }

  test("Data Evolution: V2 DELETE and UPDATE work with commit.force-compact") {
    withSparkSQLConf("spark.paimon.write.use-v2-write" -> "true") {
      withTable("t") {
